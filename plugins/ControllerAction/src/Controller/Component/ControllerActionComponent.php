...
class ControllerActionComponent extends Component
{
    ...
    public function startup(Event $event)
    {
        ...
        if($controller->request->is('ajax') && ($controller->request->is('post') || $controller->request->is('delete'))) {
    ...
    public function shutdown(Event $event)
    {
        ...
        $isBase64 = $this->request->query('base64');
        $controller = $event->subject();
        $action = $this->request->action;
        $excludedActions = [
            'getEditButtonByPlugin'
        ];

        if (isset($controller->viewVars['data']) && $refererAction && !$isBase64 && !in_array($action, $excludedActions) && $this->request->is('ajax')) {
            $result = $this->getAjaxResponse($controller);
            $this->response->body(json_encode($result, JSON_UNESCAPED_UNICODE | JSON_PARTIAL_OUTPUT_ON_ERROR));
    ...
    {
        if (isset($controller->viewVars['getResponseAjax'])) {
            return $controller->viewVars['getResponseAjax']->getArrayCopy();
        }
        ...
        if ($view && $view->ControllerAction && $model && $model->fields) {
            $tableHeaders = $view->ControllerAction->getTableHeaders($model->fields, $model->alias(), $dataKeys);
        }

        $result = [
            'data_response' => isset($controller->viewVars['data']) ? $controller->viewVars['data'] : [],
            'data_response_header' => $tableHeaders,
            'indexButtons' => isset($controller->viewVars['indexButtons']) ? $controller->viewVars['indexButtons'] : [],
            'toolbarButtons' => isset($controller->viewVars['toolbarButtons']) ? $controller->viewVars['toolbarButtons'] : [],
            'tabElements' => isset($controller->viewVars['tabElements']) ? $controller->viewVars['tabElements'] : [],
            'tabSelected' => isset($controller->viewVars['selectedAction']) ? $controller->viewVars['selectedAction'] : '',
            'navigations' => $navigations,
            'options' => ['limit' => 10],
            'byControllerAction' => true,
            'ajax' => 3
        ];
        ...
        foreach ($result['tabElements'] as $key => &$tabElement) {
            $tabElement['active'] = $key == $result['tabSelected'] || $tabElement['url']['controller'] == $result['tabSelected'] || count($result['tabElements']) == 1;
        }

        if ($action == 'edit' && $this->request->is('post') && $controller->viewVars['data']!= null) {
            $result['data_response_errors'] = $controller->viewVars['data']->errors();
        }
        if ($action == 'edit' && $this->request->is('get') && !empty($controller->viewVars['data'])) {
            ...
            $form.= $view->ControllerAction->renderInputElements($model);
            $form.= $view->ControllerAction->getFormButtons();
        ...
        if ($action == 'index') {
            ...
            $displayAction = is_array($result['indexButtons']) ? count($result['indexButtons']) : $result['indexButtons']->count() > 0;
            foreach ($result['data_response'] as $row) {
                $roww = $view->ControllerAction->getTableRow($row, $dataKeys, $searchableFields->getArrayCopy());

                if ($displayAction) {
                    $event = $view->ControllerAction->dispatchEvent($model, $eventKey, null, [$row, $result['indexButtons']->getArrayCopy()]);
                ...
                $newData[] = $roww;
                foreach ($row['rowActions'] as &$button) {
                    $button['urlBuild'] = Router::url($button['url'], true);
                }
            }
            $result['data_response'] = $newData;
            $result['paging'] = null;
