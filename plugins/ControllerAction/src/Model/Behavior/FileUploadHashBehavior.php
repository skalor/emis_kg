...
class FileUploadHashBehavior extends Behavior
{
    ...
    public function afterAction(Event $event)
    {
        if (isset($this->_table->fields[$this->config('content')])) {
            ...
            $comment .= '<br/>* ' . sprintf(__('Format Supported: %s'), $this->fileTypesForView2());
    ...
    public function addEditBeforePatch(Event $event, Entity $entity, ArrayObject $data, ArrayObject $options)
    {
        ...
        if ($entity->isNew()) {
            if (!empty($file) && $file['error'] == 0) { // success
                ...
                if ($this->uploadedFileIsAllowed2($file)) {
                    if ($this->uploadedFileSizeIsAcceptable2($file)) {
                        $parseUploadData = $this->parseUpload2($file);
                        $session->write($model->registryAlias().'.parseUpload', $parseUploadData);
                        $data = $this->parseUploadInput2($data, $parseUploadData);
                    } else {
                    ...
                    $entity->errors($fileContentField, ['Only the following formats are allowed: ' . $this->fileTypesForView2()]);
                    unset($data[$model->alias()][$fileContentField]);
                }
                ...
                $session->delete($model->registryAlias().'.parseUpload');
                $this->unsetProperties2($entity, $data);
            } else {
            ...
            if ($contentEditable) {
                if (!empty($file) && $file['error'] == 0) { // success
                     ...
                     */
                    $parseUploadData = $this->parseUpload2();
                    $session->write($model->registryAlias().'.parseUpload', $parseUploadData);
                    $data = $this->parseUploadInput2($data, $parseUploadData);
                } else {
                    /**
     ...
     **
     ******************************************************************************************************************/
    private function unsetProperties2($entity, $data)
    {
        /**
    ...
    private function readableFormatToBytes2()
    {
        $KILO = 1024;
        $MEGA = $KILO * 1024;
        $GIGA = $MEGA * 1024;
    ...
    public function uploadedFileIsAllowed2($file)
    {
        $isValid = true;
    ...
    public function uploadedFileSizeIsAcceptable2($file)
    {
        $isValid = true;
        $restrictedSize = $this->readableFormatToBytes2();
    ...
    private function parseUpload2($file = null)
    {
        if (!is_null($file)) {
    ...
    private function parseUploadInput2($data, $nameContentArray)
    {
        $fileNameField = $this->config('name');
        $fileContentField = $this->config('content');
        $model = $this->_table;
    ...
    public function fileTypesForView2()
    {
        return implode(', .', array_keys($this->allowableFileTypes));
    }

    public function getFileTypeForView2($filename)
    {
    ...
    public function getFileType2($ext)
    {
     ...
     */
    public function getActualFile2($phpResourceFile)
    {
        $file = '';
