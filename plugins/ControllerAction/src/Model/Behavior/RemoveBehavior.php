...
class RemoveBehavior extends Behavior
{
    ...
    public function afterAction(Event $event, ArrayObject $extra)
    {
        ...
        else if ($model->actions('remove') == 'deduction' && $model->action == 'remove') {
            $entity = $extra['entity'];
            $model->fields = [];
            $deletedValue = '';
            if ($entity->has('name')) {
    ...
    public function remove(Event $mainEvent, ArrayObject $extra)
    {
        ...
        if (!$request->is(['delete']) && $model->actions('remove') == 'deduction' ) {
            $extra['toolbarButtons']['back']['url'] = $model->url('index', 'QUERY');
        }
        ...
        else if (!$request->is(['delete']) && !$forceDeleteRecord && $model->actions('remove') == 'deduction' )
        {
            ...
            if ($model->exists($idKeys)) {
                ...
                $expenditure = TableRegistry::get('Institution.InstitutionExpenditure');
                $entrance = TableRegistry::get('Institution.InstitutionEntrance');

                $sumAmountEntrance = 0;
                $sumAmountExpenditure = 0;

                if (!is_null($entity->institution_reason_for_entrance_id)){

                    foreach ($entrance->find()->where([
                        'id !='.$entity->id,
                        'institution_reason_for_entrance_id' => $entity->institution_reason_for_entrance_id,
                ...
                if ($minAmount > 0) {
                    $model->Alert->error('general.delete.deductionDelete');
                    $this->recordHasAssociatedRecords = true;
                }
                else {
                    // Change the method to delete if the record can be deleted
