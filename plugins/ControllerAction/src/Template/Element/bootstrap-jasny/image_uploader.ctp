<script type="text/javascript">
  $(function(){ 
    ...
    if($('#existingImage').length) {
      ...
      currentStyle = currentStyle.replace('max-height', 'max-height');
      currentStyle = currentStyle.replace('max-width', 'max-width');
...
<div class="input string" >
  ...
  <div class="fileinput fileinput-new fileinput-preview" data-provides="fileinput">
    <div class="fileinput-new thumbnail" style="max-width: <?= $defaultWidth; ?>px; max-height: <?= $defaultHeight; ?>px;" id="toggleImage">
      <?= $src ?>
