...
class ControllerActionHelper extends Helper
{
    ...
    {
        if ($columnNumber > 26) {
            $columnLetter = $this->getRussianLetters(Chr(intval(($columnNumber - 1) / 26) + 64)) . $this->getRussianLetters(Chr((($columnNumber - 1) % 26) + 65));
        } else {
            $columnLetter = $this->getRussianLetters(Chr($columnNumber + 64));
        }
        return $columnLetter;
    }

    function getRussianLetters($columnNumber){
        //$letters = ['А','Б','В','Г','Д','Е','Ж','З','И','К','Л','М','Н','О','П','Р','С','Т','У','Ф','Х','Ц','Ч','Ш','Щ','Э','Ю','Я'];
        $letters = ['A'=>'А','B'=>'Б','C'=>'В','D'=>'Г','E'=>'Д','F'=>'Е','G'=>'Ж','H'=>'З','I'=>'И','J'=>'К','K'=>'Л','L'=>'М',
            'M'=>'Н','N'=>'О','O'=>'П','P'=>'Р','Q'=>'С','R'=>'Т','S'=>'У','T'=>'Ф','U'=>'Х','V'=>'Ц','W'=>'Ч','X'=>'Ш','Y'=>'Щ','Z'=>'Э'];
    ...
    {
        return [
            'select' => '<div class="input-select-wrapper c"><select name="{{name}}" {{attrs}}>{{content}}</select></div>',
    ...
    public function getFormButtons()
    {
        ...
        $buttons[] = [
            'name' => __('Cancel'),
            'attr' => ['class' => 'btn btn-outline btn-cancel hj', 'escape' => false],
        ...
        $html = '';
        if ($buttons->count() > 0) {
            $html = '<div class="form-buttons fg"><div class="button-label"></div>';
    ...
    public function getEditElementsAjax(Entity $data, $model)
    {
        ...
        foreach ($displayFields as $_field => $attr) {
            ...
            if ($visible) {
                ...
                //}
                $responseFieldsAttr[$_field] = array('type'=>$_type, 'attr'=>$_fieldAttr, 'option'=>$options);
            }
        }
        return $responseFieldsAttr;
    }
