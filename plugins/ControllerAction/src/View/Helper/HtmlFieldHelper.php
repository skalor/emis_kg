...
class HtmlFieldHelper extends Helper
{
    ...
    public $includes = [
        'datepicker' => [
            ...
            'js' => [
                'ControllerAction.../plugins/datepicker/js/bootstrap-datepicker.min',
                'ControllerAction.../plugins/datepicker/locales/bootstrap-datepicker.ru.min',
                'ControllerAction.../plugins/datepicker/locales/bootstrap-datepicker.kg.min'
            ],
    ...
    public function binary($action, Entity $data, $attr, $options = [])
    {
        $value = '';
        $table = TableRegistry::get($attr['className']);
        if($attr['field'] == 'file_content_hash') {
            $fileUpload = $table->behaviors()->get('FileUploadHash');
        }else{
        ...
        if ($action == 'index' || $action == 'view') {
            ...
            if($attr['field'] == 'file_content_hash'){
                $action = ['action' => $request->action, 'download_hash', $this->ControllerAction->paramsEncode($ids)];
            }else{
