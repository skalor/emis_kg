...
class FieldOptionBehavior extends Behavior {
    ...
    public function buildValidator(Event $event, Validator $validator, $name)
    {
        ...
        $addUniqueInternationalCode= false;
        if ($validator->hasField('international_code')) {
            $set = $validator->field('international_code');
            if (!$set->rule('ruleUnique')) {
                $addUniqueInternationalCode = true;
            }
        } else {
            $addUniqueInternationalCode = true;
        }

        if ($addUniqueInternationalCode) {
            $validator
                ->add('international_code', [
                    'ruleUnique' => [
                        'rule' => 'validateUnique',
                        'provider' => 'table',
                        'message' => __('This field has to be unique')
    ...
    public function beforeAction(Event $event, ArrayObject $extra)
    {
        ...
        $model->field('international_code',['type' => ($model->action == 'edit') ? 'readonly' : 'string', 'after' => 'default']);
