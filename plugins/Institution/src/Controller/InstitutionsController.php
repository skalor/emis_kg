...
class InstitutionsController extends AppController
{
    ...
    {
        $this->ControllerAction->process(['alias' => __FUNCTION__, 'className' => 'Institution.MonLicensing']);
    }
    // MonFounders
    public function Founders()
    {
        $this->ControllerAction->process(['alias' => __FUNCTION__, 'className' => 'Institution.InstitutionFounders']);
    }

    // AdditionallyReports
    public function AdditionallyReports()
    {
        $this->ControllerAction->process(['alias' => __FUNCTION__, 'className' => 'Institution.InstitutionAdditionallyReports']);
    }

    // AggregatedDataDoo
    public function AggregatedDataDoo()
    {
        $this->ControllerAction->process(['alias' => __FUNCTION__, 'className' => 'Institution.InstitutionAggregatedDataDoo']);
    }

    // AggregatedDataSpo
    public function AggregatedDataSpo()
    {
        $this->ControllerAction->process(['alias' => __FUNCTION__, 'className' => 'Institution.InstitutionAggregatedDataSpo']);
    }

    // AggregatedDataVpo
    public function AggregatedDataVpo()
    {
        $this->ControllerAction->process(['alias' => __FUNCTION__, 'className' => 'Institution.InstitutionAggregatedDataVpo']);
    }

    // InstitutionAttendance
    public function InstitutionAttendance()
    {
        $this->ControllerAction->process(['alias' => __FUNCTION__, 'className' => 'Institution.InstitutionAttendance']);
    }
    
    // MonFaculties
    public function Faculties()
    {
        $this->ControllerAction->process(['alias' => __FUNCTION__, 'className' => 'Institution.InstitutionFaculties']);
    }
    ...
    {
        $this->ControllerAction->process(['alias' => __FUNCTION__, 'className' => 'Institution.InstitutionSignedDocuments']);
    }
    ...
    {
        $this->ControllerAction->process(['alias' => __FUNCTION__, 'className' => 'Institution.FixedAssets']);
    }
    ...
    {
        $this->ControllerAction->process(['alias' => __FUNCTION__, 'className' => 'Institution.InstitutionScholarships']);
    }
    public function InstitutionExpenditure()
    {
        $this->ControllerAction->process(['alias' => __FUNCTION__, 'className' => 'Institution.InstitutionExpenditure']);
    }
    public function InstitutionEntrance()
    {
        $this->ControllerAction->process(['alias' => __FUNCTION__, 'className' => 'Institution.InstitutionEntrance']);
    }
    public function InstitutionEstimate()
    {
        $this->ControllerAction->process(['alias' => __FUNCTION__, 'className' => 'Institution.InstitutionEstimate']);
    }
    public function InstitutionSpc()
    {
        $this->ControllerAction->process(['alias' => __FUNCTION__, 'className' => 'Institution.InstitutionSpc']);
    }
    ...
    public function InstitutionExaminationGak()
    {
        $this->ControllerAction->process(['alias' => __FUNCTION__, 'className' => 'Institution.InstitutionExaminationGak']);
    }
    ...
    {
        $this->ControllerAction->process(['alias' => __FUNCTION__, 'className' => 'Institution.Partners']);
    }
    ...
    public function updateInstitutionCoordinate(){
        ...
        if ($institution)
        {
            $institution->longitude = isset($_GET['longitude']) ? $_GET['longitude'] : $institution->longitude;
            $institution->latitude = isset($_GET['latitude']) ? $_GET['latitude'] : $institution->latitude;
            $institutionTable->save($institution);
...
//    public function InstitutionStudentReasonForUpdate(){
//        $institutionStudents=TableRegistry::get('Institution.Students');
//        $students = $institutionStudents
//            ->find()
//            ->select($institutionStudents)
//            ->select(['student_id_count' => $institutionStudents->find()->func()->count('student_id')])
//            ->group($institutionStudents->aliasField('student_id'))
//            ->having(['student_id_count' => 1])
//            ->offset(1600000)
//            ->limit(100000)
//            ->all();
//        foreach($students as $student){
//            if (
//                $student->academic_period_id == 31 &&
//                $student->institution_reason_for_transfer_id == null &&
//                $student->student_status_id == 1
//            ){
//                $student->institution_reason_for_transfer_id = 1;
...
//            ->find()
//            ->select(['project_capacity','design_capacity'])
//            ->where([
//                'OR' => [
//                    ['project_capacity <' => 0],
//                    ['design_capacity <' =>0]
...
//            dump('do',$institution);
//            if ($institution->project_capacity < 0 && !is_null($institution->project_capacity) )$institution->project_capacity *= -1;
//            if ($institution->design_capacity < 0 && !is_null($institution->design_capacity) )$institution->design_capacity *= -1;
//            dump('posle',$institution);
//        }
//
//
//        die();
...
//    {
//
//        //$doo = TableRegistry::get('Institution.Doo');
//        //$nsvpo = TableRegistry::get('Institution.Nsvpo');
//        $school = TableRegistry::get('Institution.School');
//        $institutions = TableRegistry::get('Institution.Institutions');
//        $monMtb = TableRegistry::get('Institution.MonMtb');
//        $dooList = $school->find()->all();
//        foreach ($dooList as $item) {
//            $institutionRecord = $institutions->find()->where(['code' => $item->OKPO_code])->first();
//            if ($institutionRecord) {
//                $recordInCurrentYear = $monMtb->find()->where(['academic_period_id' => 31, 'institution_id' => $institutionRecord->id,])->first();
//                $recordInLastYear = $monMtb->find()->where(['academic_period_id' => 30, 'institution_id' => $institutionRecord->id,])->first();
//
//                if ($recordInCurrentYear) {
//
//                    $recordInCurrentYear->number_of_computers = $item->comps_amnt;
//                    $recordInCurrentYear->working_computers = $item->working_comps;
//                    $recordInCurrentYear->number_of_computers_connected_to_the_internet = $item->comps_with_ntrnt;
//                    $recordInCurrentYear->number_of_projectors = $item->projector_total;
//                    $recordInCurrentYear->total_number_of_books = $item->books_in_found_total;
//                    $recordInCurrentYear->books_by_specialty = $item->lib_found;
//
//                    $monMtb->save($recordInCurrentYear);
//                }
//                else if (!$recordInCurrentYear) {
//                    $entity = $monMtb->newEntity();
//
//                    $entity->number_of_computers = $item->comps_amnt;
//                    $entity->working_computers = $item->working_comps;
//                    $entity->number_of_computers_connected_to_the_internet = $item->comps_with_ntrnt;
//                    $entity->number_of_projectors = $item->projector_total;
//                    $entity->total_number_of_books = $item->books_in_found_total;
//                    $entity->books_by_specialty = $item->lib_found;
//
//                    $entity->academic_period_id = 31;
//                    $entity->institution_id = $institutionRecord->id;
//
//                    $monMtb->save($entity);
//                }
//
//                if ($recordInLastYear) {
//
//                    $recordInLastYear->number_of_computers = $item->comps_amnt;
//                    $recordInLastYear->working_computers = $item->working_comps;
//                    $recordInLastYear->number_of_computers_connected_to_the_internet = $item->comps_with_ntrnt;
//                    $recordInLastYear->number_of_projectors = $item->projector_total;
//                    $recordInLastYear->total_number_of_books = $item->books_in_found_total;
//                    $recordInLastYear->books_by_specialty = $item->lib_found;
//
//                    $monMtb->save($recordInLastYear);
//                }
//                else if (!$recordInLastYear) {
//                    $entity = $monMtb->newEntity();
//
//                    $entity->number_of_computers = $item->comps_amnt;
//                    $entity->working_computers = $item->working_comps;
...
//        $school = TableRegistry::get('Institution.School');
//        $institutions = TableRegistry::get('Institution.Institutions');
//        $aTypes = TableRegistry::get('Institution.AssetTypes');
//
//        $dooList = $school->find()->all();
//        $assetTypes = $aTypes->find('list', ['keyField' => 'international_code', 'valueField' => 'id'])->toArray();
//        foreach ($dooList as $item) {
//            $institutionRecord = $institutions->find()->where(['code' => $item->OKPO_code])->first();
//            if ($institutionRecord) {
//                    $have='бар / есть';
//                    if($item->sewage == $have){
//                        $this->getAssetRecordByTypeId($assetTypes['SEWERAGE AVAILABILITY'],$institutionRecord->id);
//                    }
//                    if($item->hot_water == $have){
//                        $this->getAssetRecordByTypeId($assetTypes['HOT WATER SUPPLY AVAILABILITY'],$institutionRecord->id);
//                    }
//                    if($item->toilets == $have){
//                        $this->getAssetRecordByTypeId($assetTypes['AVAILABILITY TOILET'],$institutionRecord->id);
//                    }
//                    if($item->access_to_fresh_wtr == $have){
//                        $this->getAssetRecordByTypeId($assetTypes['ACCESS TO DRINKING WATER'],$institutionRecord->id);
//                    }
//                    if($item->access_to_hnd_clnr == $have){
//                        $this->getAssetRecordByTypeId($assetTypes['ACCESS TO WASHSTANDS'],$institutionRecord->id);
//                    }
//                    if($item->med_staff == $have){
//                        $this->getAssetRecordByTypeId($assetTypes['MEDICAL PERSONNEL AVAILABILITY'],$institutionRecord->id);
//                    }
//                    if($item->dinning_hall == $have){
//                        $this->getAssetRecordByTypeId($assetTypes['CANTEEN'],$institutionRecord->id);
//                    }
//                    if($item->digit_lib == $have){
//                        $this->getAssetRecordByTypeId($assetTypes['DIGITAL LIBRARY AVAILABILITY'],$institutionRecord->id);
//                    }
//                    if($item->panduses == $have){
//                        $this->getAssetRecordByTypeId($assetTypes['RAMPS AVAILABILITY'],$institutionRecord->id);
//                    }
//            }
//        }
//
...
//    }
//
//    public function getAssetRecordByTypeId($typeId, $institutionId)
//    {
//        $assets = TableRegistry::get('Institution.InstitutionAssets');
//        $AcademicPeriods = TableRegistry::get('AcademicPeriod.AcademicPeriods');
//        $aTypes = TableRegistry::get('Institution.AssetTypes');
//        if ($typeId) {
//            $assetRecord = $assets->find()->where(['asset_type_id' => $typeId, 'institution_id' => $institutionId,'academic_period_id' => $AcademicPeriods->getCurrent()])->first();
//            if(!$assetRecord){
//                $type = $aTypes->get($typeId);
//                if (!$type) {
//                    return null;
//                }
//
//                $entity = $assets->newEntity();
//
//                $entity->asset_type_id = $typeId;
//                $entity->institution_id = $institutionId;
//                $entity->academic_period_id = $AcademicPeriods->getCurrent();
//                $entity->code = __($type->name);
//                $entity->name = __($type->name);
//                $entity->asset_condition_id = 1;
//                $entity->accessibility = 1;
//                $entity->asset_status_id = 1;
//                $entity->purpose = ($type->international_code == "DIGITAL LIBRARY AVAILABILITY") ? 1 : 0;
//
...
//    public function updateInstitutionContactPersons()
//    {
//
//        $institutionContactPersons = TableRegistry::get('Institution.InstitutionContactPersons');
//        $positionTitle = TableRegistry::get('Institution.StaffPositionTitles');
//        $staffTable = TableRegistry::get('Institution.Staff');
//        $institutions = TableRegistry::get('Institution.Institutions')->find()->all();
//        foreach ($institutions as $institution) {
//
//            $cods=['1'=>'MGR', '13'=>'MGR', '2'=>'DIRECTOR', '3'=>'RECTOR', '4'=>'RECTOR','6'=>'RECTOR'];
//            if (array_key_exists($institution->institution_type_id,$cods)){
//                $positionTitles = $positionTitle
//                    ->find('list')
//                    ->select(['id'])
//                    ->where(['international_code' => $cods[$institution->institution_type_id]])
//                    ->toArray();
//                $staffRecord = $staffTable
//                    ->find()
//                    ->where([
//                        $staffTable->aliasField('institution_id') => $institution->id,
//                        $staffTable->aliasField('staff_status_id') => 1,
//                    ])
//                    ->matching('Positions', function ($q) use ($positionTitles) {
//                        return $q->where([
//                            'Positions.staff_position_title_id IN ' => array_keys($positionTitles),
//                            'Positions.status_id' => 29]);
//                    })
//                    ->contain('Positions')
//                    ->contain('Users')
//                    ->first();
//
//                if ($staffRecord) {
//                    $institutionContactPersons->updateAll(['preferred' => 0], ['institution_id' => $institution->id]);
//
//                    $entity = $institutionContactPersons->newEntity();
//
//                    $entity->contact_person = $staffRecord->user->last_name . ' ' . $staffRecord->user->first_name . ' ' . $staffRecord->user->middle_name;
//                    if (!is_null($staffRecord->position)) $entity->position_id = $staffRecord->position->staff_position_title_id;
//                    $entity->mobiile_number = $staffRecord->user->mobile_phone;
//                    $entity->email = $staffRecord->user->email;
//                    $entity->preferred = 1;
//                    $entity->institution_id = $institution->id;
//
//                    $institutionContactPersons->save($entity);
//
//                }
//            }
//
...
//    }
    ...
    {
        $this->ControllerAction->process(['alias' => __FUNCTION__, 'className' => 'Institution.Correspondence']);
    }
    // End
    ...
    public function Classes($subaction = 'index', $classId = null)
    {
        if ($subaction == 'edit') {
            ...
            $classRecord=TableRegistry::get('Institution.InstitutionClasses')->get($classId);

            $this->set('classLanguages', TableRegistry::get('Languages')->find('list')->all()->toArray());
            $this->set('language_id',$classRecord->language_id);

            $this->set('classViews', TableRegistry::get('Institution.InstitutionClassesView')->getOptionList($institutionId));
            $this->set('institution_classes_view_id',$classRecord->institution_classes_view_id);
    ...
    public function Subjects($subaction = 'index', $institutionSubjectId = null)
    {
        if ($subaction == 'edit') {
            ...
            $this->set('institutionId', $institutionId);
            $this->set('advanced_study', TableRegistry::get('Institution.InstitutionSubjects')->get($institutionSubjectId['id'])->advanced_study);
    ...
    public function getListByPlugin(){

        $response = $this->ControllerAction->process(['alias' => __FUNCTION__, 'className' => 'Institution.Students']);
        return new Response(['body' => json_encode($response)]);
        //return new Response(['body' => json_encode($response)]);
    }

    public function saveByPlugin(){
        $response = $this->ControllerAction->process(['alias' => __FUNCTION__, 'className' => 'Institution.StudentUser']);
        return new Response(['body' => json_encode($response)]);
    }

    public function getViewByPlugin(){
        $response = $this->ControllerAction->process(['alias' => __FUNCTION__, 'className' => 'Institution.StudentUser']);
        return new Response(['body' => json_encode($response)]);
        //return new Response(['body' => json_encode($this->request)]);
    }

    public function getEditButtonByPlugin(){

        $response = $this->ControllerAction->processGetEditButton(['alias' => __FUNCTION__, 'className' => 'Institution.StudentUser']);
        return new Response(['body' => json_encode($response)]);
        //return new Response(['body' => json_encode($this->request)]);
    }

    public function getEditByPlugin(){
        $response = $this->ControllerAction->process(['alias' => __FUNCTION__, 'className' => 'Institution.StudentUser']);
        // удаляем дубликат
        /*unset($response['entity']);
        // конвертируем не итерируемый объект в массив
        $response['data_response'] = $response['data_response']->toArray();
        $response = $this->convertFileTree($response);*/

        return new Response(['body' => json_encode($response)]);
    }
    function convertFileTree($var){
        if( is_iterable($var) ) {
            $result = array();
            foreach ($var as $k => $v) {
                $result[$k] = $this->convertFileTree($v);
            }
            return $result;
        }
        else
            return is_resource($var) ? base64_encode($var) : $var;
    }
    ...
    public function StudentsAjax($pass = 'index')
    {
        if ($pass == 'add') {
            $session = $this->request->session();
            $roles = [];
    ...
    public function updateOpenEmisIdByUser(){
        $this->loadModel('User.Users');
        $result = $this->Users->find('all')
            ->where(['openemis_no IS NULL'])
            ->limit(400)
            ->toArray();
        ...
        if(count($result) > 0) {
            foreach ($result as $key => $item) {
                $userId = $item->id;
                if ($userId) {
                    $openemis_no = $this->Users->getUniqueOpenemisId();
                    $item->openemis_no = $openemis_no;
    ...
    public function dashboard($id)
    {
        ...
        /*$InstitutionClasses = TableRegistry::get('Institution.InstitutionClasses');
        $classesArray = $InstitutionClasses->getClassesByInstitution($id, $currentPeriod);
        $this->set('classes', $classesArray);*/

        $userModel = $this->request->session()->read('Auth.User');
        $institution_encode =  $this->ControllerAction->paramsEncode(['id' => $id]);
        $InstitutionClasses = TableRegistry::get('Institution.InstitutionClasses');
        $classesArray = $InstitutionClasses->getClassesByInstitution($id, $userModel);
        $this->set('classes', $classesArray);
        $this->set('institution_encode', $institution_encode);
    ...
    //    Meerbek's code
    public function highChartApi() {
        header('Content-type: application/json');
        $yearId = $this->request->query['yearId'];
        $session = $this->request->session();
        $id = $session->read('Institution.Institutions.id');
        ...
        echo json_encode($highChartDatas,JSON_PARTIAL_OUTPUT_ON_ERROR);
        die;
    }
    //    Meerbek's code ends
    public function getModalOptions($type)
    {
        ...
        if ($type == 'remove') {
            ...
            $modal['form'] = [
                'model' => null,
                'formOptions' => ['type' => 'delete', 'url' => '#', 'ng-submit'=>'InstitutionStudentController.onClickStudentTrash($event)'],
    ...
    public function getInstitutionPositions($institutionId, $fte, $startDate, $endDate, $notInJson = false )
    {
        if ($endDate == 'null') {
            $endDate = null;
        }
        ...
        return ($notInJson) ?  json_decode($this->response,true)  : $this->response;
    }
