...
class IndividualPromotionTable extends ControllerActionTable
{
    ...
    private function setupFields(Entity $data)
    {
        ...
        $this->field('institution_reason_for_transfer_id', ['entity' => $data]);
        ...
        $this->setFieldOrder([
            ...
            'new_information_header', 'student_status_id', 'academic_period_id', 'education_grade_id', 'institution_reason_for_transfer_id', 'institution_class_id', 'effective_date']);
    }
    ...
    public function onUpdateFieldInstitutionReasonForTransferId(Event $event, array $attr, $action, Request $request)
    {
        ...
        switch ($action) {
            case 'reconfirm':
                ...
                if ($currentData->has('institution_reason_for_transfer_id')) {
            ...
            case 'add':
                $studentStatusId = !empty($request->data[$this->alias()]['student_status_id']) ? $request->data[$this->alias()]['student_status_id']: null;
                $reasonCode=false;
    ...
    private function checkIsOverStudentClassCapacity($classId)
    {
        if (!empty($classId)) {
            ...
            if (!empty($overCapacityClass)) {
                $this->Alert->clear();
                $this->Alert->show( __('Next class').' ' . $overCapacityClass['name'] .' ' . __('does not have enough capacity for students.'),'error',['reset' => true]);
                return true;
            }
        }
        return false;
    ...
    public function savePromotion(Entity $entity)
    {
        ...
        $studentObj['institution_reason_for_transfer_id'] = $entity->institution_reason_for_transfer_id;
