...
class InstitutionBuildingsTable extends ControllerActionTable
{
    ...
    {
        $this->field('total_area', ['visible' => false]);
        $this->field('number_of_places_in_the_hostel', ['visible' => false]);
        $this->field('taking_into_account_the_norms_for_lowz', ['visible' => false]);
    ...
    private function changeBuildingsType()
    {
        $types = TableRegistry::get('Infrastructure.BuildingTypes');
        if($types->get($this->get($this->paramsDecode($this->request->pass[1])['id'])->building_type_id)->international_code == "HOSTEL")
        {
            $this->field('total_area', ['visible' => true,'after'=>'building_type_id']);
            $this->field('number_of_places_in_the_hostel', ['visible' => true,'after'=>'total_area']);
            $this->field('taking_into_account_the_norms_for_lowz', ['visible' => true,'after'=>'number_of_places_in_the_hostel']);
        }
        return true;
    }
    ...
    public function onUpdateFieldNewBuildingType(Event $event, array $attr, $action, Request $request)
    {
        ...
        if (array_key_exists($this->alias(), $request->data)) {
            if (array_key_exists('building_type_id', $request->data[$this->alias()])) {
                if ($types->get($request->data[$this->alias()]['building_type_id'])->international_code == "HOSTEL")
                {
                    $this->field('total_area', ['visible' => true,'after'=>'building_type_id']);
                    $this->field('number_of_places_in_the_hostel', ['visible' => true,'after'=>'total_area']);
                    $this->field('taking_into_account_the_norms_for_lowz', ['visible' => true,'after'=>'number_of_places_in_the_hostel']);
    ...
    private function setupFields(Entity $entity)
    {
        ...
        if($this->action =='edit')$this->changeBuildingsType();
