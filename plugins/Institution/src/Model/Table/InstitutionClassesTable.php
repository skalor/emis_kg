...
//            'type' => 'select',
//            'options' => $this->Languages->find('list')->all()->toArray(),
...
//        ]);
        ...
        $this->field('institution_classes_view_id', [
            'type' => 'select',
            'after' => 'language_id',
            'attr'=>['label' => __('Types of classes / groups')],
    ...
    ******************************************************************************************************************/
    ...
    public function getClassesByInstitution($institutionId, $userModel){
        $classes = array();

        $currentAcademicPeriod = $this->AcademicPeriods->getCurrent();
        $is_staff = $userModel['is_staff'];
        ...
        //if($is_staff) {
            ...
            $condition = [$this->aliasField('institution_id') => $institutionId, $this->aliasField('academic_period_id') => $currentAcademicPeriod];



            $classesArray = $this->find()
                ->select(['InstitutionClasses.name','InstitutionClasses.total_female_students','InstitutionClasses.total_male_students','InstitutionClasses.staff_id', 'InstitutionClasses.id', 'EducationGrades.name', 'EducationGrades.id', 'EducationGrades.education_stage_id', 'Staff.last_name', 'Staff.first_name'])
                ->join([$joinClassGrades])
                ->join([$joinEducationGrade])
                ->where([
                    $condition
                ])
                ->contain(['Staff'])
            ...
            //print_r($classesArray);die;
            if (count($classesArray) > 0) {
                foreach ($classesArray as $key => $item) {
                    $education_grade_name = $item->EducationGrades['name'];
                    $education_grade_id = $item->EducationGrades['id'];
                    $education_stage_id = $item->EducationGrades['education_stage_id'];
                    $result[$education_grade_id][$education_grade_name][] = array(
                        'education_stage_id' => $education_stage_id,
                        'id' => $item->id,
                        'name' => $item->name,
                        'total_male_students' => $item->total_male_students,
                        'total_female_students' => $item->total_female_students,
                        'staff' => $item->staff
                    );
                }
            }
            //print_r($result);die;
            foreach($result as $grade => $subclasses) {
                foreach($subclasses as $grade2 => $subclasses2) {
                    $classes[$grade] = [
                        'education_stage_id' => $subclasses2[0]['education_stage_id'],
                        'edication_grade_id' => $grade,
                        'name' => $grade2,
                        'values' => $subclasses2
                    ];
                }
    ...
    /*public function getClassesByInstitution($institutionId, $userModel){
        ...
        if($is_staff) {
            ...
            $currentUserId = $userModel['id'];
            $condition = [$this->aliasField('institution_id') => $institutionId, $this->aliasField('academic_period_id') => $currentAcademicPeriod,
                $this->aliasField('staff_id')=>$currentUserId];
            //if ($is_staff) {
            //$conditionOrWhere[$this->aliasField('secondary_staff_id')] = $currentUserId;
            //}
            //print_r($condition);die;
            $classesArray = $this->find()
                ->select(['InstitutionClasses.name', 'InstitutionClasses.id', 'EducationGrades.name', 'EducationGrades.id'])
                ->join([$joinClassGrades])
                ->join([$joinEducationGrade])
                ->where([
                    $condition
            ...
            if (count($classesArray) > 0) {
                foreach ($classesArray as $key => $item) {
                    $education_grade_name = $item->EducationGrades['name'];
                    $education_grade_id = $item->EducationGrades['id'];
                    $result[$education_grade_id][$education_grade_name][] = array('id' => $item->id, 'name' => $item->name);
                }
            }
            //print_r($result);die;
            foreach($result as $grade => $subclasses) {
    ...
    ******************************************************************************************************************/
    ...
    public function addAfterAction(Event $event, Entity $entity, ArrayObject $extra)
    {
        ...
        $this->fields['institution_classes_view_id']['options'] = $this->InstitutionClassesView->getOptionList($institutionId);
    ...
    ******************************************************************************************************************/
    ...
    protected function tooltipMessage()
    {
        ...
        $message =  __("Capacity must not exceed ") . ' ' . $maxCapacity . ' ' . __(" students per class");
