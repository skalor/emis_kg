...
class InstitutionGradesTable extends ControllerActionTable
{
    ...
    public function initialize(array $config)
    {
        ...
        $this->belongsTo('InstitutionFaculties',             ['className' => 'Institution.InstitutionFaculties', 'foreignKey' => 'institution_faculty_id']);
    ...
    public function beforeAction(Event $event, ArrayObject $extra) {
        $this->field('institution_faculty_id', ['visible' => false]);
        ...
        $this->field('education_grade_id', ['sort' => ['field' => 'EducationGrades.name']]);
        if ($this->action == 'index'){
            $this->field('programme',['sort' => ['field' => 'EducationProgrammes.name']]);
        }
        //$this->field('end_date', ['onChangeReload' => true,'sort' => ['field' => 'InstitutionGrades.end_date']]);
        if(!$this->AccessControl->isAdmin()){
            $this->field('end_date', ['type' => 'disabled', 'sort' => ['field' => 'InstitutionGrades.end_date']]);
        }else{
            $this->field('end_date', ['onChangeReload' => true,'sort' => ['field' => 'InstitutionGrades.end_date']]);
        }
    }
    ...
    public function afterAction(Event $event, ArrayObject $extra)
    {
        ...
        if ($this->action == 'add'){
            $this->field('programme', ['attr'=>['class'=>'selectpicker', 'data-live-search'=>true]]);
        }else{
        ...
        if ($this->action == 'add') {
            ...
            $this->setFieldOrder([
                'level', 'institution_faculty_id', 'programme','education_grade_id', 'start_date',
...
******************************************************************************************************************/
    public function indexBeforeQuery(Event $event, Query $query, ArrayObject $extra)
    {
        ...
        $sortList = ['InstitutionGrades.start_date','InstitutionGrades.end_date','EducationGrades.name','EducationProgrammes.name'];
...
******************************************************************************************************************/
            ...
            if (empty($errors) || count($errors)==1) {
                ...
                if ($data->offsetExists('grades')) {
                    ...
                    if ($data['grades']['education_grade_id'] != 0
                        ...
                        $grade['institution_faculty_id'] = $entity->institution_faculty_id;
    ...
    public function editBeforeSave(Event $event, Entity $entity, 
        ...
        if (count($data['grades']['education_grade_subject_id']) > 0
            ...
            $countInstitutionClassGrades = count($institutionClassGrades);
            if ($countInstitutionClassGrades > 0){
    ...
    public function addAfterAction(Event $event, Entity $entity, ArrayObject $extra)
    {
        ...
        $academicPeriodObj = $AcademicPeriods->get($currentPeriod);
        $startDate = $AcademicPeriods->getDate($academicPeriodObj->start_date);
        ...
        if($startDate >= $dateOpened->format('Y-m-d')){
            $dateOpened = new Time(date("Y", strtotime($startDate)).'-'.date("m", strtotime($startDate)).'-'. date("d", strtotime($startDate)));
        }
...
******************************************************************************************************************/
    ...
    function getFacultiesOptions(){
        // Get the current time.
        $currTime = Time::now();
        $InstitutionFaculties = TableRegistry::get('Institution.InstitutionFaculties');
        $query = $InstitutionFaculties->find('list', ['valueField' => 'name'])
            ->where(['institution_id' => $this->institutionId, 'OR' => [
...
******************************************************************************************************************/
    ...
    public function onUpdateFieldLevel(Event $event, array $attr, $action, Request $request)
    {
        ...
        if ($action == 'add') {
            ...
            $this->showFacultyField($EducationCycles->get($EducationProgrammes->get($EducationGrades->get($result->education_grade_id)->education_programme_id)->education_cycle_id)->education_level_id);
        }

        return $attr;
    }

    public function showFacultyField($levelId){
        if ($levelId) {
            $EducationLevels = TableRegistry::get('Education.EducationLevels');
            $education_level_isced_id = $EducationLevels->get($levelId)->education_level_isced_id;
            ...
            if ($education_level_isced_name == 'BACHELOR OR EQUIVALENT') {
                $this->field('institution_faculty_id', ['visible' => true]);
                $this->fields['institution_faculty_id']['options'] = $this->getFacultiesOptions();
            }
        }
    }
    ...
    public function onUpdateFieldProgramme(Event $event, array $attr, $action, Request $request)
    {
        if ($action == 'add') {
            ...
            $programmeId = $attr['value'];
            $EducationProgrammes = TableRegistry::get('Education.EducationProgrammes');
            $programmeRecord = $EducationProgrammes->findById($programmeId)->first();
            $education_cycle_id = $programmeRecord->education_cycle_id;

            $EducationCycles = TableRegistry::get('Education.EducationCycles');
            $cycleRecord = $EducationCycles->findById($education_cycle_id)->first();
            /*if($cycleRecord->education_level_id != 15){
                $this->field('institution_faculty_id', ['visible' => false]);
            }else{
                $this->fields['institution_faculty_id']['options'] = $this->getFacultiesOptions();
            }*/
