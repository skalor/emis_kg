...
class InstitutionsTable extends ControllerActionTable
{
    ...
    public function initialize(array $config)
    {
         ...
         * fieldOption tables
         */
        $this->belongsTo('DurationOfWork', ['className' => 'FieldOption.DurationOfWork', 'foreignKey' => 'doo_duration']);
        ...
        $this->belongsTo('DooAppointment', ['className' => 'FieldOption.DooAppointment', 'foreignKey' => 'institution_doo_purpose']);
        $this->belongsTo('OperatingMode', ['className' => 'Institution.OperatingMode', 'foreignKey' => 'institution_working_hours_id']);
        $this->belongsTo('SchoolType', ['className' => 'Institution.SchoolType', 'foreignKey' => 'institution_school_type_id']);
        ...
        $this->hasMany('MonMtb', ['className' => 'Institution.MonMtb', 'dependent' => true, 'cascadeCallbacks' => true]);
        ...
        $advancedSearchFieldOrder = [
            'shift_type', 'classification', 'area_id', 'area_administrative_id', 'area_aimak_id', 'institution_locality_id', 'institution_type_id',
        ...
        $this->addBehavior('OpenEmis.CustomMap');
    ...
    public function validationDefault(Validator $validator)
    {
        ...
        $validator
            ...
            ->allowEmpty('area_aimak_id')
            ->add('area_aimak_id', 'ruleConfiguredAreaAdministrative', [
                'rule' => ['checkConfiguredArea']
            ])
            ...
            ->allowEmpty('old_name', function ($context) {
                return ($context['data']['change_name'] == 1) ? false : true;
            })
            ->allowEmpty('project_capacity')
            ->add('project_capacity', 'minValue', [
                'rule' => function ($value) {
            ...
            ->allowEmpty('design_capacity')
            ->add('design_capacity', 'minValue', [
                'rule' => function ($value) {
                    if ($value < 0) return __("Should not be lesser than").' 0';
    ...
    public function beforeAction(Event $event, ArrayObject $extra)
    {
        ...
        $this->field('institution_overhaul');
        ...
        $this->field('institution_working_hours_id', ['type' => 'select']);
        $this->field('institution_view_id', ['type' => 'select']);
        $this->field('institution_school_type_id', ['type' => 'select', 'visible' => false]);
        $this->field('institution_doo_purpose', ['type' => 'select', 'visible' => false]);
        $this->field('doo_duration', ['type' => 'select', 'visible' => false]);
        $this->field('logo_content', ['visible' => false,'default'=>null]);
        ...
        //$this->field('area_aimak_id', ['type' => 'areapicker', 'source_model' => 'Area.Areas', 'displayCountry' => false]);
        $this->field('area_aimak_id', ['visible' => false]);
        ...
        $this->field('design_capacity', ['after' =>'number_of_laboratories']);
        ...
        $this->field('change_name', ['options' => $this->getSelectOptions('general.yesno') ,'visible' => false]);
        $this->field('old_name', ['visible' => false]);
        ...
        if ($this->action == 'edit') {
            ...
            if(!$this->AccessControl->isAdmin()) {
                ...
                $this->field('date_closed', ['type' => 'disabled']);
...
//        }
        ...
        $this->field('board_of_trustee');
    }

    public function onUpdateFieldBoardOfTrustee(Event $event, array $attr, $action, Request $request)
    {
    ...
    ******************************************************************************************************************/
    public function viewBeforeAction(Event $event, ArrayObject $extra)
    {
        $this->setFieldOrder([
            'information_section',
            //'logo_content',
            'name', 'alternative_name', 'code','stat_gked','stat_gked', 'pin', 'classification', 'institution_sector_id', 'institution_provider_id', 'institution_type_id',
            'institution_ownership_id', 'institution_working_hours_id', 'institution_overhaul', 'institution_view_id', 'institution_gender_id', 'date_opened', 'date_closed', 'institution_status_id',
    ...
    {
        if ($entity['change_name'] == 1) $this->field('old_name', ['visible' => true,'after'=>'name']);
    ...
    ******************************************************************************************************************/
    ...
    public function editBeforeAction(Event $event, ArrayObject $extra)
    {
        $this->setFieldOrder([
            'information_section',
            //'logo_content',
            'name', 'alternative_name', 'code','stat_gked', 'pin', 'classification', 'institution_sector_id', 'institution_provider_id', 'institution_type_id',
            'institution_ownership_id', 'institution_working_hours_id', 'institution_overhaul', 'institution_view_id', 'institution_gender_id', 'date_opened', 'date_closed', 'institution_status_id',
    ...
    {
        if ($this->action == 'edit') $this->field('change_name', ['visible' => true]);
        if ($entity['change_name'] == 1) $this->field('old_name', ['visible' => true]);
        ...
        $this->field('order_date', ['default_date' => false]);
        $this->field('date_of_initial_registration', ['default_date' => false]);

        $this->setFieldOrder([
            'information_section',
            //'logo_content',
            'name', 'old_name', 'change_name', 'alternative_name', 'code','stat_gked', 'pin', 'classification', 'institution_sector_id', 'institution_provider_id', 'institution_type_id',
            'institution_ownership_id', 'institution_working_hours_id', 'institution_overhaul', 'institution_view_id', 'institution_gender_id', 'date_opened', 'date_closed', 'institution_status_id',
        ...
        if ($this->action == 'edit'){
            if(!$this->AccessControl->isAdmin()) {
                $this->fields['institution_gender_id']['type'] = 'readonly';
                $genderTable = TableRegistry::get('Institution.Genders');
                if($entity->institution_gender_id){
                    $this->fields['institution_gender_id']['attr']['value'] = $genderTable->get($entity->institution_gender_id)->name;
                }
            }
        }
    }

    public function onUpdateFieldInstitutionOverhaul(Event $event, array $attr, $action, Request $request)
    {
        $attr['options'] = $this->getSelectOptions('general.yesno');
        return $attr;
    }

    public function onUpdateFieldInstitutionViewId(Event $event, array $attr, $action, Request $request)
    {
        $attr['options']=[];
        if ($action == 'edit' || $action == 'add' ) {
            if (array_key_exists($this->alias(), $request->data)) {
                if (array_key_exists('institution_type_id', $request->data[$this->alias()])) {
                    $attr['options'] = $this->InstitutionView->getOptionList($request->data[$this->alias()]['institution_type_id']);
                }
            }
            else{
                if ($action == 'edit') {
                    $institutionRecord=$this->get($this->Session->read('Institution.Institutions.id'));
                    $attr['options']=$this->InstitutionView->getOptionList($institutionRecord->institution_type_id);
                }
            }
        }
        return $attr;
    ...
    public function onUpdateFieldChangeName(Event $event, array $attr, $action, Request $request)
    {
        $attr['onChangeReload'] = 'changeIndefinite';
        if ($request->is(['post', 'put']) || $action == 'edit') {
            if (array_key_exists($this->alias(), $request->data)) {
                if (array_key_exists('change_name', $request->data[$this->alias()]) && array_key_exists('name', $request->data[$this->alias()])) {
                    $result=$this->get($this->paramsDecode($request->pass[1])['id']);
                    if ($request->data[$this->alias()]['change_name'] == 1 )
                    {
                        if (!empty($request->data[$this->alias()]['name']) && !empty($request->data[$this->alias()]['old_name'])){
                            return $attr;
                        } else{
                            $this->field('old_name', ['attr'=>[ 'value' => $result->name],'visible'=>true ]);
                            $this->field('name', ['attr'=>[ 'value' => ''], 'order'=>2 ]);
                        }
                    }  else {
                        $this->field('old_name', ['attr'=>[ 'value' => ''], 'visible'=>false ]);
                        $this->field('name', ['attr'=>[ 'value' => $result->name] ]);
                    }
    ...
    ******************************************************************************************************************/
    ...
    public function onUpdateFieldInstitutionTypeId(Event $event, array $attr, $action, Request $request)
    {
        if ($this->request->is(['post', 'put'])){
            if ($action == 'add' || $action == 'edit') {
                if (!empty($request['data'][$this->alias()]['institution_type_id'])) {
                    $institution_type_id = $request['data'][$this->alias()]['institution_type_id'];
                    $this->showDooPurposeField($institution_type_id);
                    $this->showSchoolTypeField($institution_type_id);
                }
            }
        }else{
            if ($action == 'edit' || $action == 'view'){
                $id = $this->paramsDecode($this->request->pass[1])['id'];
                $institution_type_id = $this->get($id)->institution_type_id;
                $this->showDooPurposeField($institution_type_id);
                $this->showSchoolTypeField($institution_type_id);
            }
        }
    ...
    function showSchoolTypeField($institution_type_id){
        $InstitutionTypes = TableRegistry::get('Institution.Types');
        $international_code = $InstitutionTypes->get($institution_type_id)->international_code;
        if ($international_code == 'PRIMARY SECONDARY'){
            $this->field('institution_school_type_id', ['type' => 'select', 'visible' => true, 'after' => 'institution_type_id']);
            $this->fields['institution_school_type_id']['order'] = 9;
        }
    }

    function showDooPurposeField($institution_type_id){
        $InstitutionTypes = TableRegistry::get('Institution.Types');
        $international_code = $InstitutionTypes->get($institution_type_id)->international_code;
        if ($international_code == 'PRESCHOOL EDUCATIONAL ORGANIZATION') {
            $this->field('institution_doo_purpose', ['type' => 'select', 'visible' => true, 'after' => 'institution_type_id']);
            $this->fields['institution_doo_purpose']['order'] = 9;

            $this->field('doo_duration', ['type' => 'select', 'visible' => true, 'after' => 'institution_type_id']);
            $this->fields['doo_duration']['order'] = 10;
        }
    }
        ...
        $query
            ...
            ->formatResults(function (ResultSetInterface $results) {
                ...
                foreach ($formattedResults as $key => &$obj) {
                    ...
                    $title = __($institutionTypes[$key]) . ' ('.$numberOfRecords.')';
