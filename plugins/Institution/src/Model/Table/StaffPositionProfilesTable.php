...
use Cake\I18n\Date;
use Cake\I18n\Time;
use Institution\Controller\InstitutionsController;
...
class StaffPositionProfilesTable extends ControllerActionTable
{
    ...
    public function validationDefault(Validator $validator)
    {
        ...
        return $validator
            ->allowEmpty('end_date')
            ->remove('start_date')
            ->allowEmpty('fte',function ($context){
                $staffChangeTypes = $this->staffChangeTypesList;
                $typeId = $context['data']['staff_change_type_id'];
                if ($typeId == $staffChangeTypes['CHANGE_IN_FTE'] || $typeId == $staffChangeTypes['CHANGE_IN_STAFF_POSITION'] ) {
                    return false;
                }
                return true;
            })
            ->requirePresence('staff_change_type_id')
            ->allowEmpty('staff_type_id',function ($context){
                $staffChangeTypes = $this->staffChangeTypesList;
                if ($context['data']['staff_change_type_id'] == $staffChangeTypes['CHANGE_IN_STAFF_TYPE']) {
                    return false;
                }
                return true;
            })
            ->allowEmpty('reason_for_dismissal_id',function ($context){
                $staffChangeTypes = $this->staffChangeTypesList;
                if ($context['data']['staff_change_type_id'] == $staffChangeTypes['END_OF_ASSIGNMENT']) {
                    return false;
                }
                return true;
            })
    ...
    public function initialize(array $config)
    {
        ...
        $this->belongsTo('ReasonForDismissal', ['className' => 'Institution.ReasonForDismissal']);
...
//        $newEntity->date_of_operation=$today;
    ...
    {
        if ($this->action == 'view') {
            $oldValue=$this->Positions->get($entity->institution_staff->institution_position_id)->name;
            $newValue = $this->Positions->get($entity->institution_position_id)->name;
            if ($newValue != $oldValue) {
                return $this->getStyling(__($oldValue), __($newValue));
            } else {
                return __($newValue);
    ...
    public function beforeAction(Event $event, ArrayObject $extra)
    {
        ...
        $this->field('reason_for_dismissal_id', ['type' => 'select', 'after' => 'staff_change_type_id','visible' => ['view' => true, 'edit' => true, 'add' => true]]);
    ...
    public function addEditAfterAction(Event $event, Entity $entity, ArrayObject $extra)
    {
        ...
        $this->field('fte', ['type' => 'select', 'options' => $this->getSelectOptions('StaffPositionProfiles.FTE'), 'default' => $entity->fte]);
        $this->field('current_FTE', ['before' => 'fte', 'type' => 'disabled', 'options' => $this->getSelectOptions('StaffPositionProfiles.FTE')]);
        $this->field('effective_date');
        $this->field('end_date');
        if ($this->Session->check('Institution.StaffPositionProfiles.staffRecord')) {
            $staffChangeTypes = $this->staffChangeTypesList;
            $staffEntity=$this->Session->read('Institution.StaffPositionProfiles.staffRecord');
            $position=$this->Positions->get($this->getEntityProperty($staffEntity, 'institution_position_id'));
            $this->field('institution_position_id', [
                'after' =>($this->request->data[$this->alias()]['staff_change_type_id'] == $staffChangeTypes['CHANGE_IN_STAFF_POSITION']) ? 'fte' :'staff_id',
                'type' => 'readonly',
                'attr' => ['value' => $position->name],
                'default' => $position->id
            ]);
        }
    ...
    public function onUpdateFieldReasonForDismissalId(Event $event, array $attr, $action, Request $request)
    {
        if ($this->Session->check('Institution.StaffPositionProfiles.staffRecord')) {
            $entity = $this->Session->read('Institution.StaffPositionProfiles.staffRecord');
            $staffChangeTypes = $this->staffChangeTypesList;
            if ($action == 'add' || $action == 'edit') {
                if ($request->data[$this->alias()]['staff_change_type_id'] == $staffChangeTypes['END_OF_ASSIGNMENT']) {
                    $attr['type'] = 'select';
                    $options = $this->ReasonForDismissal->getList()->toArray();
                    if (isset($options[$entity->reason_for_dismissal_id])) {
                        unset($options[$entity->reason_for_dismissal_id]);
                    }
                    $attr['options'] = $options;
                } else {
                    $attr['type'] = 'hidden';
                    $attr['value'] = $entity->reason_for_dismissal_id;
                }
            } elseif ($action == 'view') {
                if ($staffChangeTypes['END_OF_ASSIGNMENT'] != $this->get($this->paramsDecode($request->pass[1])['id'])->staff_change_type_id) {
                    $attr['visible'] = false;
                }
            }
        }
        $attr['after'] = 'staff_change_type_id';
        return $attr;
    }
    ...
    public function onUpdateFieldFte(Event $event, array $attr, $action, Request $request)
    {
        if ($action == 'add' || $action == 'edit') {
            $staffChangeTypes = $this->staffChangeTypesList;
            if (isset($request->data[$this->alias()])) {
                if ($request->data[$this->alias()]['staff_change_type_id'] == $staffChangeTypes['CHANGE_IN_FTE'] || $request->data[$this->alias()]['staff_change_type_id'] == $staffChangeTypes['CHANGE_IN_STAFF_POSITION']) {
                    $attr['type'] = 'select';
                    if (isset($attr['options'])) {
                        $options = $attr['options'];
                        if ($this->Session->check('Institution.StaffPositionProfiles.staffRecord') && $request->data[$this->alias()]['staff_change_type_id'] != $staffChangeTypes['CHANGE_IN_STAFF_POSITION'] ) {
                            $entity = $this->Session->read('Institution.StaffPositionProfiles.staffRecord');
    ...
    public function onUpdateFieldEffectiveDate(Event $event, array $attr, $action, Request $request)
    {
        if ($action == 'add' || $action == 'edit') {
            $staffChangeTypes = $this->staffChangeTypesList;
            $typeId=$request->data[$this->alias()]['staff_change_type_id'];
            if ($typeId == $staffChangeTypes['CHANGE_IN_FTE'] || $typeId == $staffChangeTypes['CHANGE_IN_STAFF_POSITION'] ) {
                $attr['type'] = 'date';
                if ($this->Session->check('Institution.StaffPositionProfiles.staffRecord')) {
                    $entity = $this->Session->read('Institution.StaffPositionProfiles.staffRecord');
    ...
    public function onUpdateFieldInstitutionPositionId (Event $event, array $attr, $action, Request $request)
    {
        if ($action == 'add' || $action == 'edit') {
            $staffChangeTypes = $this->staffChangeTypesList;
            if ($request->data[$this->alias()]['staff_change_type_id'] == $staffChangeTypes['CHANGE_IN_STAFF_POSITION'] && !empty($request->data[$this->alias()]['fte'])) {
                $attr['type']='select';
                $controller= new InstitutionsController();
                $institutionId=$request->data[$this->alias()]['institution_id'];
                $fte=$request->data[$this->alias()]['fte'];
                $startDate=$request->data[$this->alias()]['effective_date'];
                $endDate=$request->data[$this->alias()]['end_date'];
                $attr['options']=$controller->getInstitutionPositions($institutionId, $fte, $startDate, $endDate, true);
            }
        }
        return $attr;
    }
