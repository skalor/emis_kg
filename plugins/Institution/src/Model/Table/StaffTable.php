...
class StaffTable extends ControllerActionTable
{
    ...
    public function indexBeforeQuery(Event $event, Query $query, ArrayObject $extra)
    {
        ...
        // Staff Position Titles Types options

        $positionTitlesTypesOptions=$this->getSelectOptions('Staff.position_types');
        $positionTitlesTypesOptions=[ 3 => __('All Types')]+$positionTitlesTypesOptions;

        $selectedTitlesType = !is_null($this->request->query('position_titles_type')) ? $this->request->query('position_titles_type') : 3;

        if ($selectedTitlesType != 3) {
            $query->matching('Positions', function ($q) use ($selectedTitlesType) {
                $StaffPositionTitlesIds=TableRegistry::get('Institution.StaffPositionTitles')
                    ->find('list')
                    ->select(['id'])
                    ->where(['type' => $selectedTitlesType])
                    ->toArray();
                return $q->where(['Positions.staff_position_title_id IN ' =>array_keys($StaffPositionTitlesIds)]);
            });
        }
        ...
        $this->controller->set(compact('periodOptions', 'positionOptions', 'statusOptions', 'positionTitlesTypesOptions', 'selectedTitlesType'));
    }
    ...
    public function afterAction(Event $event, ArrayObject $extra)
    {
        ...
        $this->fields['staff_id']['sort'] = ['field' => 'Users.last_name'];
