...
class StudentPromotionTable extends AppTable
{
    ...
    public function validationDefault(Validator $validator)
    {
        ...
        return $validator
            ...
            ->allowEmpty('next_class', function ($context) {
                $studentStatusId = (!empty($context['data']['student_status_id']))? $context['data']['student_status_id']: '';
                return ($studentStatusId == $this->statuses['GRADUATED']);
            });
    }
    ...
    public function addAfterAction(Event $event, Entity $entity)
    {
        ...
        $this->ControllerAction->field('institution_reason_for_transfer_id', [
            'attr' => [
                'label' => __('Institution Reason For Transfer')
            ],
            'entity' => $entity
        ]);
        ...
        $this->ControllerAction->setFieldOrder([
            ...
            'education_grade_id',
            'next_class',
            'institution_reason_for_transfer_id',
            'students'
        ]);
    }
    ...
    public function onUpdateFieldInstitutionReasonForTransferId(Event $event, array $attr, $action, Request $request)
    {
        $attr['type'] = 'readonly';
        $attr['visible'] = false;
        $InstitutionReasonForTransfer = TableRegistry::get('Institution.InstitutionReasonForTransfer');
        switch ($action) {
            case 'reconfirm':
                $sessionKey = $this->registryAlias() . '.confirm';
                if ($this->Session->check($sessionKey)) {
                ...
                if ($currentData->has('institution_reason_for_transfer_id') && !empty($currentData->institution_reason_for_transfer_id)) {
                    $reason = $InstitutionReasonForTransfer
                        ->find()
                        ->where([$InstitutionReasonForTransfer->aliasField('id') => $currentData->institution_reason_for_transfer_id])
                        ->select([$InstitutionReasonForTransfer->aliasField('name')])
                        ->first();
                    $reasonName = (!empty($reason)) ? $reason->name : '';
                    $attr['visible'] = true;
                }
                $attr['attr']['value'] = (!empty($reasonName)) ? __($reasonName) : '';
                break;
            case 'add':
                $entity = $attr['entity'];
                ...
                switch ($studentStatusId) {
                    case $this->statuses['PROMOTED']:
                        $reasonCode = 'PROMOTION';
                        break;
                    case $this->statuses['REPEATED'];
                        $reasonCode = 'REPEATED YEAR';
                        break;
                    case $this->statuses['GRADUATED']:
                        if (!empty($entity->next_class)) {
                            $reasonCode = 'PROMOTION';
                        }
                        break;
                }
                if ($reasonCode) {
                    $reason = $InstitutionReasonForTransfer
                        ->find()
                        ->where([$InstitutionReasonForTransfer->aliasField('international_code') => $reasonCode])
                        ->first();
                    $reasonName = ($reason) ? $reason->name : 'No available classes';
                    $attr['value'] = ($reason) ? $reason->id : null;
                    $attr['visible'] = true;
                }
                $attr['attr']['value'] = (!empty($reasonName)) ? __($reasonName) : '';
                break;
    ...
    public function onUpdateToolbarButtons(Event $event, ArrayObject $buttons, ArrayObject $toolbarButtons, array $attr, $action, $isFromModel)
    {
        switch ($action) {
            case 'add':
                ...
                $toolbarButtons['back']['label'] = '<svg class="backSvg" width="23" height="24" viewBox="0 0 23 24" fill="none" xmlns="http://www.w3.org/2000/svg"> <path d="M23 10.4999H5.50563L13.5412 2.11501L11.5 0L0 12L11.5 24L13.5268 21.8849L5.50563 13.4999H23V10.4999Z" fill="#004A51"></path> </svg>';
    ...
    public function savePromotion(Entity $entity, ArrayObject $data)
    {
        ...
        $institution_reason_for_transfer_id = null;
        $currentGrade = null;
        $statusToUpdate = null;
        $studentStatuses = $this->statuses;
        $institutionId = $this->institutionId;
        ...
        if (array_key_exists('institution_reason_for_transfer_id', $data[$this->alias()])) {
            $institution_reason_for_transfer_id = $data[$this->alias()]['institution_reason_for_transfer_id'];
        }
                    ...
                    if ($studentObj['selected']) {
                        ...
                        if ($saveAsDraft) {
                            ...
                            if ($nextAcademicPeriodId != 0) {
                                ...
                                $studentObj['institution_reason_for_transfer_id'] = $institution_reason_for_transfer_id;
        ...
        if (!empty($entity)) {
            ...
            if (!empty($overCapacityClasses)) {
                ...
                foreach ($overCapacityClasses as $class) {
                    $this->Alert->show( __('Next class ') . $class->name . __(' does not have enough capacity for students.'),'error');
                }
                return true;
            }
        }
