...
class StudentsTable extends ControllerActionTable
{
    ...
    public function initialize(array $config)
    {
         ...
         * fieldOption tables
         */
        $this->belongsTo('Reason', ['className' => 'Institution.InstitutionReasonForTransfer', 'foreignKey' => 'institution_reason_for_transfer_id']);
    ...
    public function onExcelBeforeQuery(Event $event, ArrayObject $settings, Query $query)
    {
        ...
        // MonPib
        $educationGradeId = isset($this->request->query['education_grade_id']) ? $this->request->query['education_grade_id'] : null;
        if ($educationGradeId) {
            $query->where([$this->aliasField('education_grade_id') => $educationGradeId]);
        }
        $classId = isset($this->request->query['class_id']) ? $this->request->query['class_id'] : null;
        if ($classId) {
            $query->where([$ClassStudents->aliasField('institution_class_id') => $classId]);
        }
    }
    ...
    public function indexBeforeAction(Event $event, ArrayObject $extra)
    {
        ...
        $queryStatus = !is_null($this->request->query('status_id')) ? $this->request->query('status_id') : null;
        $reasonVisible = (in_array($queryStatus,array($this->statuses['WITHDRAWN'],$this->statuses['TRANSFERRED']))) ? true : false ;
        ...
        $this->field('reason_for_the_operation',[
            'visible' => ['index' => $reasonVisible, 'view' => $reasonVisible, 'add' => false, 'edit' => false],
            'after' => 'institution_reason_for_transfer_id'
        ]);
        $this->fields['start_date']['visible'] = false;
        $this->fields['end_date']['visible'] = false;

        $this->fields['class']['sort'] = ['field' => 'InstitutionClasses.name'];
        $this->fields['student_id']['sort'] = ['field' => 'Users.last_name'];
    ...
    public function indexBeforeQuery(Event $event, Query $query, ArrayObject $extra)
    {
        ...
        $stageIds = $InstitutionEducationGrades
            ->find('list', [
                'valueField' => 'EducationGrades.education_stage_id'
            ])
            ->select([
            ...
            ->group('EducationGrades.education_stage_id')
            ->toArray();

        if ($stageIds){
            $stageOptions = $educationStages
                ->find('list')
                ->where(['id IN ' => $stageIds])
                ->order([$educationStages->aliasField('order')])
                ->toArray();
        }
        $stageOptions = ['-1' => __('All Stages')] + $stageOptions;
        $selectedEducationStages = $this->queryString('education_stage_id', $stageOptions);
        ...
        $educationGradesOptions = $InstitutionEducationGrades
            ...
            ->where([
                ...
                ($selectedEducationStages > 0) ? 'education_stage_id' : 'education_stage_id !=' => $selectedEducationStages ])
        ...
        $this->advancedSelectOptions($academicPeriodOptions, $selectedAcademicPeriod, [
            ...
            'callable' => function ($id) use ($studentTable, $institutionId, $request) {
                $studentsCount = $studentTable->find()->where(['institution_id'=>$institutionId, 'academic_period_id'=>$id])->count();
                return isset($request->query['no_students']) && $request->query['no_students'] === 'true' ? 1 : $studentsCount;
            }
        ]);
        ...
        $this->advancedSelectOptions($stageOptions, $selectedEducationStages);

        if ($selectedEducationGrades != -1) {
            $query->where([$this->aliasField('education_grade_id') => $selectedEducationGrades]);
        }

        if ($selectedEducationStages != -1) {

            $educationGradesIds = $this->EducationGrades
                ->find('list',['valueField' => 'id'])
                ->select(['id'])
                ->where(['education_stage_id' => $selectedEducationStages])
                ->toArray();
            $query->where([$this->aliasField('education_grade_id IN') => $educationGradesIds]);
        }

        $query->where([$this->aliasField('academic_period_id') => $selectedAcademicPeriod]);
        if (!empty($request->query['class_id'])){
            $query->where([$this->aliasField('InstitutionClasses.id') => $request->query['class_id']]);
        }

        $searchList = [];
        if (!empty($request->query['openemis_no'])) {
            $searchList[$this->aliasField('Users.openemis_no LIKE')] = '%'. $request->query['openemis_no'] . '%';
        }
        if (!empty($request->query['first_name'])) {
            $searchList[$this->aliasField('Users.first_name LIKE')] = '%'. $request->query['first_name'] . '%';
        }
        if (!empty($request->query['last_name'])) {
            $searchList[$this->aliasField('Users.last_name LIKE')] = '%'. $request->query['last_name'] . '%';
        }
        if (!empty($request->query['identity_number'])) {
            $searchList[$this->aliasField('Users.identity_number LIKE')] = '%'. $request->query['identity_number'] . '%';
        }
        if ($searchList) {
            $query->where(['OR' => $searchList]);
        }
        ...
        if (!isset($request->query['sort'])) {
            $query->order([$this->Users->aliasField('last_name'), $this->Users->aliasField('first_name')]);
        }

        $this->controller->set(compact('statusOptions', 'academicPeriodOptions', 'educationGradesOptions', 'stageOptions'));
    }
    ...
    public function editAfterAction(Event $event, Entity $entity)
    {
        ...
        // End PHPOE-1897
            ...
            $this->field('institution_reason_for_transfer_id', ['type' => 'select']);
    ...
    public function onGetReasonForTheOperation(Event $event, Entity $entity)
    {
        $studentWithdraw = TableRegistry::get('Institution.WithdrawRequests');
        ...
        $reasonRecord = null;
        if ($entity->student_status_id == $this->statuses['WITHDRAWN']) {
            $withdraw = $studentWithdraw
                ->find()
                ->where([
                    'student_id' => $studentId,
                    'institution_id' => $entity->institution->id,
                    'education_grade_id' => $entity->education_grade->id,
                    'academic_period_id' => $entity->academic_period->id,
                ])
                ->last();
            if ($withdraw) {
                $reasonRecord = $studentWithdrawReasons->get($withdraw->student_withdraw_reason_id);
            }
        } else if ($entity->student_status_id == $this->statuses['TRANSFERRED']) {
            $transfer = $studentTransfers
                ...
                ->last();
            if ($transfer) {
                $reasonRecord = $studentTransferReasons->get($transfer->student_transfer_reason_id);
            }
        }

        return ($reasonRecord) ? $reasonRecord->name : '';
    }
    ...
    public function getNumberOfStudentsByStage($params = [])
    {
        ...
        $studentByGrades = $query
            ...
            ->order(
                ['EducationStages.order', 'EducationLevels.order', 'EducationCycles.order', 'EducationProgrammes.order']
