				...
				<tbody>
					<?php foreach ($attr['data']['subjects'] as $i=>$obj) : ?>
						<?php
                            ...
                            $advancedSelected = (array_key_exists('existedSubjects', $attr['data']) && array_key_exists($n, $attr['data']['existedSubjects']) && $attr['data']['existedSubjects'][$n]['advanced_study'] == 1) ? 'checked' : '';
							if ($selected) {
					...
					<tr>
                        ...
                        <td <?php if(!$disabled){ ?> class="checkbox-column" <?php } ?>>
                            <?= $this->Form->checkbox(sprintf('MultiSubjects[%d][advanced_study]', $i), [
                                'value' => 1,
