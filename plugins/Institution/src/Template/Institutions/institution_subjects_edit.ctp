...
<form accept-charset="utf-8" id="content-main-form" class="form-horizontal ng-pristine ng-valid" novalidate="novalidate" ng-controller="InstitutionSubjectStudentsCtrl as InstitutionSubjectStudentsController">
    ...
    <div class="input select">
        <label><?= __('Advanced Study') ?></label>
        <div class="input-select-wrapper">
            <select name="Students[advanced_study]"
                    id="students-advanced-study"
                    ng-options="item.id as item.name for item in [{'id':1,'name':'<?= __('Yes')?>'},{'id':0,'name':'<?= __('No')?>'}]"
                    ng-model="InstitutionSubjectStudentsController.advanced_study"
                    ng-init="InstitutionSubjectStudentsController.advanced_study = <?=$advanced_study?>">
