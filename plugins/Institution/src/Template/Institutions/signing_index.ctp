...
$url = ['institutionId' => $institutionId, 'action' => 'sign'];
if ($pass) {
...
<div class="table-wrapper" ng-class="disableElement">
    <div class="table-responsive">
        <table class="<?= $tableClass ?>" <?= $displayReorder ? 'id="sortable" url="' . $baseUrl . '"' : '' ?>>
            ...
            <tbody <?= $displayAction ? 'data-link="row cvn"' : '' ?>>
                <?php echo $this->Html->tableCells($tableData) ?>
                <?php if ($data->count() >= 1) : ?>
                <tr>
                    <td class="text-right" colspan="12" colspan="12">
                        <?= $this->Form->button(__('Sign'), ['type' => 'submit', 'name' => 'submit', 'value' => 'sign', 'class' => 'hidden']) ?>
...
<?php
echo $this->Form->control('sign_history_type', ['type' => 'string', 'class' => 'hidden', 'value' => isset($signHistoryType) ? $signHistoryType : '']);
if ($institutionId) {
    echo $this->Form->control('institution_id', ['type' => 'number', 'class' => 'hidden', 'value' => $this->Signing->paramsDecode($institutionId)]);
}
echo $this->Form->end();
