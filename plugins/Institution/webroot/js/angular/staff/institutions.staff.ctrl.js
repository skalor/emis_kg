...
function InstitutionStaffController($location, $q, $scope, $window, $filter, UtilsSvc, AlertSvc, AggridLocaleSvc, InstitutionsStaffSvc) {
    ...
    StaffController.StaffLanguageOptions = [];
    ...
    StaffController.Staff.language_id = '';
    StaffController.Staff.language_name = '';
    ...
    StaffController.internalFilterPin;
    ...
    angular.element(document).ready(function () {
        ...
        .then(function(promisesObj) {
            ...
            StaffController.staffTypeOptions = translateOptions(staffTypes);
            ...
            promises[6]  = InstitutionsStaffSvc.getLanguages();
        ...
        .then(function(promisesObj) {
            StaffController.genderOptions = translateOptions(promisesObj[0]);
            ...
            if (promisesObj[3] != undefined && promisesObj[3].hasOwnProperty('data')) {
                StaffController.StaffNationalitiesOptions = translateOptions(promisesObj[3]['data'], ['identity_type']);
            }
            // User Identities
            ...
            if (promisesObj[6] != undefined && promisesObj[6].hasOwnProperty('data')) {
                StaffController.StaffLanguageOptions = translateOptions(promisesObj[6]['data']);
                angular.forEach(StaffController.StaffLanguageOptions, function(value) {
                    if(value['default'] == 1){StaffController.Staff.language_id = value['id'];}
                });
            }
        }, function(error) {
            console.log(error);
    ...
    function initNationality() {
        ...
        for(var i = 0; i < options.length; i++) {
            if (options[i].default == 1) {
                ...
                StaffController.Staff.identity_type_name = translateLabel(options[i].identity_type.name);
                break;
            }
        }
    }
        ...
        for(var i = 0; i < options.length; i++) {
            if (options[i].id == nationalityId) {
                if(options[i].identity_type_id == null){
                    ...
                    StaffController.Staff.identity_type_name = translateLabel(identityOptions['0'].name);
                }else{
                    StaffController.Staff.identity_type_id = options[i].identity_type_id;
                    StaffController.Staff.identity_type_name = translateLabel(options[i].identity_type.name);
                }
    ...
    function changeIdentityType() {
        ...
        for(var i = 0; i < options.length; i++) {
            if (options[i].id == identityType) {
                StaffController.Staff.identity_type_name = translateLabel(options[i].name);
                break;
            }
        }
    }
    ...
    function  translateOptions( options , properties = false) {
        InstitutionsStaffSvc.translate(options.map(o => o.name)).then(function (data){
            for (let i in data) {
        ...
        if (properties) {
            for (let p in properties) {
                ...
                InstitutionsStaffSvc.translate(options.map(o => o[property].name)).then(function (data) {
                    for (let i in data) {
    ...
    function  translateLabel( label ) {
        ...
        InstitutionsStaffSvc.translate(text).then(function (data){
            label = data._label;
        });
        return label;
    }
    ...
    function onChangeFTE() {
        ...
        .then(function(response) {
            StaffController.institutionPositionOptions.availableOptions = translateOptions(response);
        }, function(errors) {
        ...
        .then(function(localeText){
            StaffController.internalGridOptions = {
                columnDefs: [
                    ...
                    {headerName: StaffController.translatedTexts.pin, field: "pin", suppressMenu: true, suppressSorting: true},
                   ...
                   InstitutionsStaffSvc.getStaffRecords(
                    {
                        ...
                        conditions: {
                            ...
                            pin: StaffController.internalFilterPin,
    ...
    function onAddNewStaffClick() {
        ...
        StaffController.selectedStaffData.pin = '';
        StaffController.selectedStaffData.email = '';
        StaffController.selectedStaffData.mobile_phone = '';
    ...
    function setStaffName() {
        ...
        StaffController.appendName(staffData, 'pin', true);
        StaffController.appendName(staffData, 'email', true);
        StaffController.appendName(staffData, 'mobile_phone', true);
    ...
    function addStaffUser(staffData, academicPeriodId, institutionPositionId, positionType, fte, staffTypeId, startDate, endDate) {
        ...
        newStaffData['language_id'] = StaffController.Staff.language_id;
    ...
    function validateNewUser() {
        ...
        if(StaffController.selectedStaffData.mobile_phone != '' && !StaffController.selectedStaffData.mobile_phone.match(/996\d{9}/)){
            StaffController.postResponse.error.mobile_phone = {'_empty': 'Please enter a valid Phone'};
            remain = true;
        }

        if(StaffController.selectedStaffData.email != '' && !StaffController.selectedStaffData.email.match(/^[^\s@]+@[^\s@]+\.[^\s@]+$/)){
            StaffController.postResponse.error.email = {'_empty': 'Please enter a valid Email'};
            remain = true;
        }
        ...
        if(StaffController.selectedStaffData.pin != '' && StaffController.selectedStaffData.pin != undefined){

            var pin = StaffController.selectedStaffData.pin.trim()

            var reg =  /^\d+$/;
            if(!reg.test(pin)){
                StaffController.postResponse.error.pin = numeric;
                remain = true;
            }

            if (StaffController.Staff.nationality_id == 1 ) {
                if(pin.length != 14){
                    StaffController.postResponse.error.pin = pinMassage;
                    remain = true;
                }
            }
