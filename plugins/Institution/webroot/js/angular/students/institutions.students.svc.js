...
function InstitutionsStudentsSvc($http, $q, $filter, KdOrmSvc) {
    ...
    var service = {
        ...
        getStudentdTest: getStudentdTest,
        ...
        getClassStudentView: getClassStudentView,
        getClassStudentTabView: getClassStudentTabView,
        getClassStudentEdit: getClassStudentEdit,
    ...
    var models = {
        ...
        Reason: 'Institution.InstitutionReasonForTransfer',
        Languages: 'Languages',
    ...
    function getStudentdTest() {
        var deferred = $q.defer();
        //var url = angular.baseUrl + '/Institutions/getListByPlugin/indexajax';
        //var url = angular.baseUrl + '/Institutions/eyJpZCI6MTEzLCI1YzNhMDliZjIyZTEyNDExYjZhZjQ4ZGZlMGI4NWMyZDlkMTE4MWNkMzkxZTA4OTU3NGM4Y2YzY2ExZTVlNGFkIjoiNThlODBjZWVlYTgzNzRlYmNmODkwYjRjNmFmNTZiMGEifQ.NjFiMzlkZDEyODQwZjJlZDdjMTIxMWU0NDU1MGZhNWRiNDRkMzIzNmI3Yzk4NTQ1N2RjY2JhZjliNjMwMWRiMw/getListByPlugin/indexajax';
        var url = angular.baseUrl + '/Institutions/eyJpZCI6OTYsIjVjM2EwOWJmMjJlMTI0MTFiNmFmNDhkZmUwYjg1YzJkOWQxMTgxY2QzOTFlMDg5NTc0YzhjZjNjYTFlNWU0YWQiOiIzMTQ1ZDJjYTVkN2RjNTM5N2ZkZWQzMzMzYmRlYzE2NiJ9..MDA3NjAzZGZkYjhhMzdkMjUxMzczZDZkMThmNTQwMGRmYmJjYzExYzBlNTM5YzEzY2YwYmUxZDQwZDUwODExYg/' +
            '?academic_period_id=30&status_id=1&education_grade_id=1&queryString=eyJpbnN0aXR1dGlvbl9zdHVkZW50X2lkIjoiYWRkMDBhOGUtMzk5MS00NWNkLTg2NjEtYWQwYzgxNDU1YTI0IiwiNWMzYTA5YmYyMmUxMjQxMWI2YWY0OGRmZTBiODVjMmQ5ZDExODFjZDM5MWUwODk1NzRjOGNmM2NhMWU1ZTRhZCI6IjMxNDVkMmNhNWQ3ZGM1Mzk3ZmRlZDMzMzNiZGVjMTY2In0.YTYzNmQyOGNmNjZlMmIyNWY5MTM4MDQ4ZDUzMWU3OTM5YTI3ZWVmOTMxZjBmMjc4MWI1ZTdjYmI3ZjZiMjg2YQ';
    ...
    // fix pib
    function getClassStudents(class_id, institution_id, academic_period_id, page, search = null) {
        var deferred = $q.defer();
        var url = angular.baseUrl + '/Institutions/'+institution_id+'/StudentsAjax/index/?class_id='+class_id+'&no_students=true&academic_period_id='+academic_period_id+'&page='+page;

        if (typeof search === 'object' && search) {
            if (search.openemis_no) { url += '&openemis_no=' + search.openemis_no; }
            if (search.first_name) { url += '&first_name=' + search.first_name; }
            if (search.last_name) { url += '&last_name=' + search.last_name; }
            if (search.identity_number) { url += '&identity_number=' + search.identity_number; }
        }
    ...
    function getClassStudentNavigation(plugin, institution_id) {
        var url = angular.baseUrl + '/'+plugin+'/Institutions/'+institution_id+'/StudentProgrammes/index';
    }

    function getClassStudentView(class_id, institution_id, student_id, academic_period_id, status_id, education_grade_id, queryString) {
        var deferred = $q.defer();
        var url = angular.baseUrl + '/Institutions/'+institution_id+'/getEditButtonByPlugin/view/'+student_id+'/?academic_period_id='+academic_period_id+'&status_id='+status_id+'&education_grade_id='+education_grade_id+'&queryString='+queryString;
        $http.get(url, {
            'headers': {
                'X-Requested-With' :'XMLHttpRequest'
            }
    ...
    function getClassStudentTabView(url) {
        var deferred = $q.defer();
        url = angular.baseUrl + url;
        $http.get(url, {
            'headers': {
                'X-Requested-With' :'XMLHttpRequest'
            }
    ...
    function getClassStudentEdit(class_id, institution_id, student_id, academic_period_id, status_id, education_grade_id, queryString){
        var deferred = $q.defer();
        var url = angular.baseUrl + '/Institutions/'+institution_id+'/getEditByPlugin/edit/'+student_id+'/?academic_period_id='+academic_period_id+'&status_id='+status_id+'&education_grade_id='+education_grade_id+'&queryString='+queryString;
        $http.get(url, {
            'headers': {
                'X-Requested-With' :'XMLHttpRequest'
            }
    ...
    function saveStudentEdit(class_id, institution_id, student_id, academic_period_id, status_id, education_grade_id, fields) {
        var deferred = $q.defer();
        var params = {
            method: 'POST',
            url: angular.baseUrl + '/Institutions/'+institution_id+'/saveByPlugin/edit/'+student_id+
                '?academic_period_id='+academic_period_id+'&status_id='+status_id+'&education_grade_id='+education_grade_id,
            headers: {
    ...
    function getClassesData(options)
    {
        var success = function(response, deferred) {
            var classes = response.data.data;
            console.log('uerere',response)
            // does not matter if no classes available
            deferred.resolve(classes);
        };
        return InstitutionClasses
            .select()
            .where(options)
            .contain(['ClassGrades', 'EducationGrades', 'Staff'])
            .ajax({success: success, defer: true});
    }

    function removeStudent(institution_id, student_id, academic_period_id, status_id, education_grade_id, fields)
    {
        var deferred = $q.defer();
        var params = {
            method: 'POST',
            url: angular.baseUrl + '/Institutions/' + institution_id + '/StudentsAjax/remove' +
                '?academic_period_id=' + academic_period_id + '&status_id=' + status_id + '&education_grade_id=' + education_grade_id,
            headers: {
                'Content-Type': 'application/x-www-form-urlencoded',
                'X-Requested-With' :'XMLHttpRequest',
    ...
    function universalQuery(url, method = 'GET', fields = {})
    {
        var deferred = $q.defer();
        var queryParams = {
            url: angular.baseUrl + url,
        ...
        $http(queryParams).then(function(response){
            deferred.resolve(response.data);
        }, function(error) {
            deferred.reject(error);
        });
