...
use ControllerAction\Model\Behavior\ControllerActionBehavior;

/**
 * OpenEmis CustomMapBehavior
 *
 ...
 *
 */
class CustomMapBehavior extends Behavior
{
    ...
    public function onGetMapElement(Event $event, $action, Entity $entity, $attr, $options)
    {
        ...
        $mapConfig = [
            'institutionId'=> (array_key_exists('id', $attr)) ? $attr['id'] : $entity->id,
            'zoom' => $mapZoom,
            'attribution' => 'ISUO',
            'type' => 'basic'
        ];
        ...
        if (array_key_exists('latitude', $attr) && array_key_exists('longitude', $attr)) {
            ...
            $attr['mapPosition'] = $mapPosition;

            return $event->subject()->renderElement('OpenEmis.customMap', ['attr' => $attr]);
        }
