...
(function() {
    ...
    kdMultiSelectCtrl.$inject = ['$scope', 'KdOrmSvc'];

    function kdMultiSelectCtrl($scope, KdOrmSvc) {
        ...
        translate($scope.textConfig).then(function (dataTranslated) {
            for (let i in dataTranslated) {
                $('.translate-text').each(function () {
                    if ($scope.textConfig[i] == $(this).text()) {
                        $(this).text(dataTranslated[i]);
                    }
                });
                $scope.textConfig[i] = dataTranslated[i];
            }
        });

        var defaultGridOptions = getDefaultGridOpt($scope.textConfig);
        ...
        function getDefaultGridOpt(_textConfig) {
            return {
                primaryKey: 'id',
                headerHeight: 38,
                rowHeight: 38,
                ...
                overlayNoRowsTemplate: '<span class="ag-custom-overlay"><i class="fa fa-info-circle fa-lg margin-right-10"></i><span class="translate-text">' + _textConfig.noStudentRecordFound + '</span></span>',
        ...
        function getDefaultTextConfig() {
            return {
                ...
                noStudentRecordFound: "No student record found"
            };
        }
        ...
        function getHeaderCheckbox(_scopeObj, _textConfig) {
            ...
            return [{
                checkbox: '<span class="ag-cell-wrapper"><input type="checkbox" name="name" class="ag-select-all-' + _scopeObj.gridPosition + '"><span class="ag-cell-value"></span></span> <span class="checkbox-text"><span class="translate-text">' + _textConfig.selectAll + '</span> - <span class="ag-row-count"></span> <span <span class="translate-text">' + checkboxLabel + '</span></span>'
            }];
        }
