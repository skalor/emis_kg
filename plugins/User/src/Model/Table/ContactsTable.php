...
class ContactsTable extends ControllerActionTable
{
    ...
    public function afterSave(Event $event, Entity $entity, ArrayObject $options)
    {
        ...
        if (($entity->dirty('preferred') && $entity->preferred == 1) || $entity->preferred == 1) {
            ...
            if ($contactOption == $this->contactOptionsArray['MOBILE']){
                ...
                $this->dispatchEventToModels('Model.UserContacts.onChangePhone', [$entity], $this, $listeners);
            }
        }
    }
    ...
    private function buildBaseValidator(Validator $validator)
    {
        $validator
            ...
            ->add('value', 'ruleValidateMobilePhone', [
                'rule' => ['checkMobilePhone'],
                'on' => function ($context) {
                    $contactOptionId = (array_key_exists('contact_option_id', $context['data']))? $context['data']['contact_option_id']: null;
                    if (is_null($contactOptionId)) {
                        if (array_key_exists('contact_type_id', $context['data'])) {
                    ...
                    return ($contactOptionId == $this->contactOptionsArray['MOBILE']);
                },
            ])
            ->add('value', 'ruleValidateOtherPhone', [
                'rule' => ['checkOtherPhone'],
                'on' => function ($context) {
                    $contactOptionId = (array_key_exists('contact_option_id', $context['data']))? $context['data']['contact_option_id']: null;
                    if (is_null($contactOptionId)) {
                        if (array_key_exists('contact_type_id', $context['data'])) {
                    ...
                    return in_array($contactOptionId, [$this->contactOptionsArray['PHONE'], $this->contactOptionsArray['FAX']]);
                },
            ])
