...
//        ]);
    ...
    public function implementedEvents()
    {
        ...
        $newEvent = [
            ...
            'Model.UserContacts.onChangePhone' => 'onChangePhoneUserContacts'
        ];
    ...
    public function beforeAction(Event $event)
    {
        ...
        $this->ControllerAction->field('change_surname', ['visible' => false]);
        $this->ControllerAction->field('old_surname', ['visible' => false]);
    ...
    public function getUniqueOpenemisId($options = [])
    {
        ...
        if(!empty($resultOpenemisTemps->openemis_no)){  
           ...
           $newOpenemisNo = $resultOpenemisTemp->openemis_no + 1;
            ...
            $resultOpenemisTemps2 = $openemisTemps->find('all')
                ->where(['openemis_no' => $newOpenemisNo])
                ->first();
            if(!empty($resultOpenemisTemps2->openemis_no)) {
                $query = $openemisTemps->find();
                $openemisQuery = $query
                    ->select(['openemis_no' => $query->func()->max('openemis_no*1')]);
                $openemisQueryFirst = $openemisQuery->first();
                $newOpenemisNo = $openemisQueryFirst->openemis_no + 1;
            }
    ...
    public function validationDefault(Validator $validator)
    {
        ...
        $validator
            ...
            ->allowEmpty('change_surname')
            ->allowEmpty('old_surname')
    ...
    public function setUserValidation(Validator $validator, $thisModel = null)
    {
        $validator
            ...
            ->allowEmpty('pin')
            ->add('pin', 'rulePinCheck',[
                'rule' => function ($value, $context){
                    $context = $context['data'];
                    $result = true;
                    if ($value) {
                        if (!preg_match("/^([0-9])+$/", trim($value))){
                            $result = __('The value must be numeric.');
                        }
                        $natId = (isset($context['nationality_id']) && !empty($context['nationality_id'])) ? $context['nationality_id'] : null;
                        if (!$natId){
                            $natId = (isset($context['nationalities'][0]['nationality_id']) && !empty($context['nationalities'][0]['nationality_id'])) ? $context['nationalities'][0]['nationality_id'] : null;
                        }
                        if($natId){
                            if ($this->MainNationalities->get($natId)->international_code == 'KGZ') {
                                if(strlen(trim($value)) != 14){
                                    $result = __('The value must be numeric and must be equal to 14');
                                }
                            }
                        }
                    }
        ...
        $thisModel->setValidationCode('pin', 'User.Users');
        return $validator;
    }
    ...
    public function onChangePhoneUserContacts(Event $event, Entity $entity)
    {
        $securityUserId = $entity->security_user_id;
        $mobile_phone = $entity->value;

        // update the user phone with preferred phone
        $this->updateAll(['mobile_phone' => $mobile_phone], ['id' => $securityUserId]);
    }
}
