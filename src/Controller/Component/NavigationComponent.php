...
//            Meerbek's code / This checks for main FAQ if it is Admin
            if ($navigations['Faq.index']){
                $navigations["Faq.faq"] = array("title" => "Faq","icon" => "<span><i class='fa s fa-comments'></i></span>","params" =>array("plugin" => "Faq","selected"=>array(0 => "Faq.faq")));
                $userInfo = $this->AccessControl->getRolesByUser($controller->Auth->user('id'));
                $userRoleId = $userInfo->toArray()[0]->security_role_id;
                if (!(($this->AccessControl->isAdmin()) or ($userRoleId == 1)) ){
                    unset($navigations['Administration']);
                    unset($navigations['Faq.index']);
                }
            }
            ...
            $controller->set('profileMenu', $this->getProfileMenus());
        ...
        foreach ($navigations as $key => $value) {
            ...
            if (isset($value['link']) && !$value['link']) {
                ...
                if (array_key_exists('controller', $url) && !in_array($url['plugin'], $ignoredPlugin)) {
                    if (
                        !$this->AccessControl->check($url, $rolesRestrictedTo)
...
//            $navigations = $this->appendNavigation('Profiles.Profiles', $navigations, $this->getProfileNavigation());
        ...
        //$navigations = $this->appendNavigation('StaticReports.StaticReports', $navigations,   $this->getStatisticReportNavigation());

        return $navigations;
    }

    public function getStatisticReportNavigation()
    {
        $navigation = [
            'StatisticReport.index' => [
                'title' => __('StatisticReport'),
                'parent' => 'StatisticReport',
                'params' => ['plugin' => 'StatisticReport'],
            ],
            'StatisticReportHistory.index' => [
                'title' => __('StatisticReportHistory'),
                'parent' => 'StatisticReport',
                'params' => ['plugin' => 'StatisticReportHistory'],
                '_ext' => 'StatisticReport',
            ],
            'TemplateReport.index' => [
                'title' => 'TemplateReport',
                'parent' => __('StatisticReport'),
                'params' => ['plugin' => 'TemplateReport'],
                '_ext' => 'StatisticReport',
            ],
...
//            ],
            ...
            'Reports' => [
                'title' => 'Reports',
                'icon' => '<span><i class="fa fa-bar-chart"></i></span>',
                'link' => false,
            ],
            ...
            'Search' => [
                'title' => 'Search',
                'icon' => '<span><i class="fa fa-search"></i></span>',
                'link' => false
            ],
            'Faq.faq' => [
                'title' => 'Faq',
                'icon' => '<span><i class="fa fa-comments"></i></span>',
                'params' => ['plugin' => 'Faq'],
                'selected' => ['Faq.faq']
            ],
            /*'TemplateReport.index' => [
                'title'     => 'TemplateReport',
                'icon'      => '<span><i class="fa kd-items"></i></span>',
                'params'    => ['plugin' => 'TemplateReport'],
                'selected'  => ['TemplateReport.index']
            ],*/

            'StatisticReport' => [
                'title' => 'StatisticReport',
                'icon' => '<span><i class="fa kd-reports"></i></span>',
                'link' => false,
            ],

            // MonReports
            'MonTemplateReports.index' => [
                'title' => 'MonTemplateReports',
                'icon' => '<span><i class="fa fa-file-text-o"></i></span>',
                'params' => ['plugin' => 'MonTemplateReports'],
                'selected' => ['MonTemplateReports.view', 'MonTemplateReports.add', 'MonTemplateReports.edit', 'MonTemplateReports.delete']
            ],
            'MonStatisticReports.index' => [
                'title' => 'MonStatisticReports',
                'icon' => '<span><i class="fa kd-reports"></i></span>',
                'params' => ['plugin' => 'MonStatisticReports'],
                'selected' => ['MonStatisticReports.view', 'MonStatisticReports.add', 'MonStatisticReports.edit', 'MonStatisticReports.delete']
            ],
            'MonGeneratedStatisticReports.index' => [
                'title' => 'MonGeneratedStatisticReports',
                'icon' => '<span><i class="fa fa-print"></i></span>',
                'params' => ['plugin' => 'MonGeneratedStatisticReports'],
                'selected' => ['MonGeneratedStatisticReports.view', 'MonGeneratedStatisticReports.add', 'MonGeneratedStatisticReports.edit', 'MonGeneratedStatisticReports.delete']
            ],//*/
        ...
        $navigation = [
            ...
            'Institutions.Institutions.view' => [
                ...
                'Institutions.Licensing' => [
                    'title' => 'Licensing',
                    'parent' => 'Institution.General',
                    'selected' => ['Institutions.Licensing'],
                    'params' => ['plugin' => 'Institution'],
                ],//*/
            ...
            'InstitutionCalendars.index' => [
                ...
                'Institutions.Partners.index' => [
                    'title' => 'Partners',
                    'parent' => 'Institution.General',
                    'params' => ['plugin' => 'Institution',],
                    'selected' => ['Institutions.Parents.index', 'Institutions.Parents.view'],
                ],
            ...
            'Institutions.SignedDocuments.index' => [
                'title' => 'Signed Documents',
                'parent' => 'Institution.General',
                'selected' => ['Institutions.SignedDocuments'],
                'params' => ['plugin' => 'Institution']
            ],

            'InstitutionHistories.index' => [
                'title' => 'Histories',
                'parent' => 'Institution.General',
                'params' => ['plugin' => 'Institution', 0 => 'view', 1 => $institutionId],
                'selected' => ['InstitutionHistories.view']
            ],
            ...
            'Institutions.Founders' => [
                'title' => 'Founders',
                'parent' => 'Institution.General',
                'selected' => ['Institutions.Founders'],
                'params' => ['plugin' => 'Institution'],
            ],//*/
            ...
            // Mon AdditionallyReports
            'Institutions.AdditionallyReports' => [
                'title' => 'AdditionallyReports',
                'parent' => 'AggregatedData',
                'selected' => ['Institutions.AdditionallyReports'],
                'params' => ['plugin' => 'Institution'],
            ],

            // Mon AggregatedDataDoo
            'Institutions.AggregatedDataDoo' => [
                'title' => 'Aggregated Data Doo',
                'parent' => 'AggregatedData',
                'selected' => ['Institutions.AggregatedDataDoo'],
                'params' => ['plugin' => 'Institution'],
            ],

            // Mon AggregatedDataSpo
            'Institutions.AggregatedDataSpo' => [
                'title' => 'Aggregated Data Spo',
                'parent' => 'AggregatedData',
                'selected' => ['Institutions.AggregatedDataSpo'],
                'params' => ['plugin' => 'Institution'],
            ],

            // Mon AggregatedDataVpo
            'Institutions.AggregatedDataVpo' => [
                'title' => 'Aggregated Data Vpo',
                'parent' => 'AggregatedData',
                'selected' => ['Institutions.AggregatedDataVpo'],
                'params' => ['plugin' => 'Institution'],
            ],
            // Mon InstitutionAttendance
            'Institutions.InstitutionAttendance' => [
                'title' => 'Attendance',
                'parent' => 'AggregatedData',
                'selected' => ['Institutions.InstitutionAttendance'],
                'params' => ['plugin' => 'Institution'],
            ],
            // MonFounders
            'Institutions.Correspondence.index' => [
                'title' => 'Correspondence',
                'parent' => 'Institution.General',
                'selected' => ['Institutions.Correspondence.view','Institutions.Correspondence.add','Institutions.Correspondence.edit','Institutions.Correspondence.delete',],
                'params' => ['plugin' => 'Institution'],
            ],//*/
            'Institution.Academic' => [
                'title' => 'Academic',
            ...
            'Institutions.Faculties' => [
                'title' => 'Faculties',
                'parent' => 'Institution.Academic',
                'selected' => ['Institutions.Faculties'],
                'params' => ['plugin' => 'Institution'],
            ],//*/
            //aza
                // 'Institution.Schedules' => [
            ...
            'Institutions.InstitutionExaminationGak' => [
                'title' => 'Gak',
                'parent' => 'Institutions.Examinations',
                'params' => ['plugin' => 'Institution'],
                'selected' => ['Institutions.InstitutionExaminationGak.index', 'Institutions.InstitutionExaminationGak.view','Institutions.InstitutionExaminationGak.add','Institutions.InstitutionExaminationGak.delete','Institutions.InstitutionExaminationGak.edit'],
            ],
            ...
            'Institutions.Scholarships.index' => [
                'title' => 'Scholarships',
                'parent' => 'Institution.Finance',
                'params' => ['plugin' => 'Institution',],
                'selected' => ['Institutions.Scholarships.index', 'Institutions.Scholarships.view','Institutions.Scholarships.add','Institutions.Scholarships.delete','Institutions.Scholarships.edit'],
            ],
            'Institution.Budget' => [
                'title' => 'Budget',
                'parent' => 'Institution.Finance',
            ...
            'Institutions.InstitutionEntrance.index' => [
                'title' => 'Entrance',
                'parent' => 'Institution.Budget',
                'params' => ['plugin' => 'Institution',],
                'selected' => ['Institutions.InstitutionEntrance.index', 'Institutions.InstitutionEntrance.view','Institutions.InstitutionEntrance.add','Institutions.InstitutionEntrance.delete','Institutions.InstitutionEntrance.edit'],
            ],
            'Institutions.InstitutionExpenditure.index' => [
                'title' => 'Expenditure',
                'parent' => 'Institution.Budget',
                'params' => ['plugin' => 'Institution',],
                'selected' => ['Institutions.InstitutionExpenditure.index', 'Institutions.InstitutionExpenditure.view','Institutions.InstitutionExpenditure.add','Institutions.InstitutionExpenditure.delete','Institutions.InstitutionExpenditure.edit'],
            ],
            'Institutions.InstitutionEstimate.index' => [
                'title' => 'Estimate',
                'parent' => 'Institution.Budget',
                'params' => ['plugin' => 'Institution',],
                'selected' => ['Institutions.InstitutionEstimate.index', 'Institutions.InstitutionEstimate.view','Institutions.InstitutionEstimate.add','Institutions.InstitutionEstimate.delete','Institutions.InstitutionEstimate.edit'],
            ],
            'Institutions.InstitutionSpc' => [
                'title' => 'Institution Spc',
                'parent' => 'Institution.Finance',
                'params' => ['plugin' => 'Institution'],
                'selected' => ['Institutions.InstitutionSpc'],
            ],
            'Infrastructures' => [
            ...
            'Institutions.InstitutionLands' => [
                ...
                'MonMtb.index' => [
                    ...
                    'parent' => 'Infrastructures',
                    'params' => ['plugin' => 'Institution'],
                    'selected' => ['MonMtb.view', 'MonMtb.add', 'MonMtb.edit', 'MonMtb.delete']
                ],//*/
            ...
            // MonFixedAssets
            'Institutions.FixedAssets.index' => [
                'title' => 'FixedAssets',
                'parent' => 'Infrastructures',
                'selected' => ['Institutions.FixedAssets.view','Institutions.FixedAssets.add','Institutions.FixedAssets.edit','Institutions.FixedAssets.delete',],
                'params' => ['plugin' => 'Institution'],
            ],//*/
            'Survey' => [
                'title' => 'Survey',
    ...
    public function getInstitutionStudentNavigation()
    {
        ...
        $navigation = [
            ...
            'Institutions.StudentProgrammes.index' => [
                ...
                'parent' => 'Institutions.Students.index',
                'params' => ['plugin' => 'Institution'],
                'selected' => ['Students.Classes', 'Students.Subjects', 'Students.StudentPractice', 'Students.StudentEducation', 'Students.Absences', 'Students.Behaviours', 'Students.Results', 'Students.ExaminationResults', 'Students.ReportCards', 'Students.Awards',
            ...
            'Students.Healths' => [
                ...
                'parent' => 'Institutions.Students.index',
                'params' => ['plugin' => 'Student'],
                'selected' => ['Students.Healths', 'Students.HealthAllergies', 'Students.HealthPeopleDisabilities', 'Students.HealthConsultations', 'Students.HealthFamilies', 'Students.HealthHistories', 'Students.HealthImmunizations', 'Students.HealthMedications', 'Students.HealthTests', 'StudentBodyMasses.index', 'StudentBodyMasses.add', 'StudentBodyMasses.edit', 'StudentBodyMasses.view', 'StudentBodyMasses.delete', 'StudentInsurances.add', 'StudentInsurances.view', 'StudentInsurances.edit', 'StudentInsurances.delete', 'StudentInsurances.index']
            ],
            'Students.SpecialNeedsReferrals' => [
                'title' => 'Special Needs',
                'parent' => 'Institutions.Students.index',
                'params' => ['plugin' => 'Student'],
                'selected' => ['Students.SpecialNeedsReferrals','Students.SpecialNeedsUserAllowance', 'Students.SpecialNeedsAssessments', 'Students.SpecialNeedsServices', 'Students.SpecialNeedsDevices', 'Students.SpecialNeedsPlans', 'Students.SpecialNeedsHostel']
            ],
    ...
    public function getInstitutionStaffNavigation()
    {
        ...
        $navigation = [
            ...
            'Staff.Healths' => [
                ...
                'parent' => 'Institutions.Staff.index',
                'params' => ['plugin' => 'Staff'],
                'selected' => ['Staff.Healths', 'Staff.HealthAllergies', 'Staff.HealthPeopleDisabilities', 'Staff.HealthConsultations', 'Staff.HealthFamilies', 'Staff.HealthHistories', 'Staff.HealthImmunizations', 'Staff.HealthMedications', 'Staff.HealthTests', 'StaffBodyMasses.index', 'StaffBodyMasses.add', 'StaffBodyMasses.edit', 'StaffBodyMasses.view', 'StaffBodyMasses.delete', 'StaffInsurances.index', 'StaffInsurances.add', 'StaffInsurances.edit', 'StaffInsurances.view', 'StaffInsurances.delete']
            ],
            'Staff.SpecialNeedsReferrals' => [
                'title' => 'Special Needs',
                'parent' => 'Institutions.Staff.index',
                'params' => ['plugin' => 'Staff'],
                'selected' => ['Staff.SpecialNeedsReferrals','Staff.SpecialNeedsUserAllowance', 'Staff.SpecialNeedsAssessments', 'Staff.SpecialNeedsServices', 'Staff.SpecialNeedsDevices', 'Staff.SpecialNeedsPlans', 'Staff.SpecialNeedsHostel']
            ]
        ];
        foreach ($navigation as &$n) {
            if (isset($n['params'])) {
    ...
    public function getProfileNavigation()
    {
        $navigation = [
            ...
            'Profiles.SpecialNeedsReferrals' => [
                ...
                'parent' => 'Profiles.Profiles',
                'params' => ['plugin' => 'Profile'],
                'selected' => ['Profiles.SpecialNeedsReferrals','Profiles.SpecialNeedsUserAllowance', 'Profiles.SpecialNeedsAssessments', 'Profiles.SpecialNeedsServices', 'Profiles.SpecialNeedsDevices', 'Profiles.SpecialNeedsPlans', 'Profiles.SpecialNeedsHostel']
            ],
    ...
    public function getDirectoryNavigation()
    {
        $navigation = [
            ...
            'Directories.SpecialNeedsReferrals' => [
                ...
                'parent' => 'Directories.Directories.index',
                'params' => ['plugin' => 'Directory'],
                'selected' => ['Directories.SpecialNeedsReferrals','Directories.SpecialNeedsUserAllowance', 'Directories.SpecialNeedsAssessments', 'Directories.SpecialNeedsServices', 'Directories.SpecialNeedsDevices', 'Directories.SpecialNeedsPlans', 'Directories.SpecialNeedsHostel']
            ]
        ];
    ...
    public function getProfileStudentNavigation()
    {
        $navigation = [
            ...
            'Profiles.StudentProgrammes.index' => [
                ...
                'parent' => 'Profiles.Student',
                'params' => ['plugin' => 'Profile'],
                    'selected' => ['Profiles.StudentProgrammes.index', 'Profiles.StudentSubjects', 'Profiles.StudentPractice', 'Students.StudentEducation', 'Profiles.StudentClasses', 'Profiles.StudentAbsences', 'Profiles.StudentBehaviours',
    ...
    public function getDirectoryStudentNavigation()
    {
        ...
        $navigation = [
            ...
            'Directories.StudentProgrammes.index' => [
                ...
                'parent' => 'Directories.Student',
                'params' => ['plugin' => 'Directory'],
                    'selected' => ['Directories.StudentProgrammes.index', 'Directories.StudentSubjects', 'Directories.StudentPractice', 'Students.StudentEducation','Directories.StudentClasses', 'Directories.StudentAbsences', 'Directories.StudentBehaviours',
...
//                'params' => ['plugin' => 'Faq'],
            ...
            'FieldOptions.index' => [
                ...
                'MonFields.index' => [
                    'title' => 'MonFields',
                    'parent' => 'SystemSetup',
                    'params' => ['plugin' => 'MonFields'],
                    'selected' => ['MonFields.index', 'MonFields.add', 'MonFields.view', 'MonFields.edit', 'MonFields.delete']
                ],
                'MonSections.index' => [
                    'title' => 'MonSections',
                    'parent' => 'SystemSetup',
                    'params' => ['plugin' => 'MonFields'],
                    'selected' => ['MonSections.index', 'MonSections.add', 'MonSections.view', 'MonSections.edit', 'MonFields.delete']
                ],//*/
            ...
            'LocaleContents.index' => [
                ...
                'selected' => ['LocaleContents.index', 'LocaleContents.view', 'LocaleContents.edit']
		            ...
		            'ModuleTranslations.index' => [
                       ...
                       'parent' => 'SystemSetup.Localization',
                       'selected' => ['ModuleTranslations.index', 'ModuleTranslations.view', 'ModuleTranslations.edit']
                    ],
            ...
            'Credentials.index' => [
                ...
                'selected' => ['Credentials.view', 'Credentials.add', 'Credentials.edit', 'Credentials.delete']
                    ...
                    'MonApi.index' => [
                        'title' => 'MonAPI',
                        'parent' => 'API',
                        'params' => ['plugin' => 'MonAPI'],
                        'selected' => ['MonApi.view', 'MonApi.add', 'MonApi.edit', 'MonApi.delete']
                    ],//*/
            'Notices.index' => [
                'title' => 'Notices',
                'parent' => 'SystemSetup',
            ...
            'ApiLogs.log' => [
                'title' => 'Log',
                'parent' => 'Administration.ApiLogs',
                'selected' => ['ApiLogs.index'],
                'params' => ['plugin' => 'ApiLogs', 'controller' => 'ApiLogs', 'action' => 'index']
            ]
        ];
        return $navigation;
    }

    private function getProfileMenus(): array
    {
        $profileMenu = $this->getProfileNavigation();
        $navs = [
        ...
        $session = $this->request->session();
        $userId = $this->controller->paramsEncode(['id' => $session->read('Auth.User.id')]);
        foreach($profileMenu as $action => $nav) {
            $action = explode('.', $action);
            $navs[$nav['title']] = ['plugin' => 'Profile', 'controller' => 'Profiles', 'action' => $action[1]];

            if(isset($action[2])) {
                $navs[$nav['title']][0] = $action[2];
                if ($userId){
                    $navs[$nav['title']][1] = $userId;
                }
            }
        }
        return $navs;
