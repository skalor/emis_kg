...
******************************************************************************************************************/
		    	...
		    	if (!in_array($field, $this->_exclude) && $entity->has($field)) {
					if (!is_null($schema->column($field)) && !in_array($schema->columnType($field), $this->_excludeType)) {
		    			...
		    			if ($oldValue != 'World' && $oldValue != $newValue) {
							...
							if ($track) {
                                $obj['institution_id'] = $this->_session->read('Institution.Institutions.id');
								$obj['operation'] = 'edit';
    ...
    public function afterSave(Event $event, Entity $entity) {
        ...
        if ($entity->id && $this->_table->trackActivity && $request->action === 'add') {
            $activity = [
                'model' => $this->_table->alias(),
                'model_reference' => $entity->id,
                'operation' => $request->action,
                'institution_id' => $this->_session->read('Institution.Institutions.id'),
                $this->config('key') => $entity->{$this->config('keyField')}
            ];
