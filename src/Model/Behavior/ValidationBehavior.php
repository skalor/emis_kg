...
class ValidationBehavior extends Behavior
{
    ...
    public static function checkMobilePhone($check, array $globalData){
        $match = preg_match('/^996\d{9}$/', $check);
        return !empty($match);
    }

    public static function checkOtherPhone($check, array $globalData){
        $match = preg_match('/^\d*$/', $check);
        return !empty($match);
    }
    ...
    public static function checkConfiguredArea($check, array $globalData)
    {
        ...
        //echo $check . '/' . $validateAreaAdministrativeLevel;die;
        ...
        if ($globalData['field'] == 'area_id') {
            ...
            $checkLevel = $AreaLevels->get($Areas->get($check)->area_level_id)->level;

            if ($checkLevel != $validateAreaLevel) {
                ...
                $Areas = TableRegistry::get('Area.Areas');
                $parent_id = $Areas->get($check)->parent_id;

                $area_administrative_id = $globalData['data']['area_administrative_id'];
                $AreaAdministratives = TableRegistry::get('Area.AreaAdministratives');
                $codeAdministratives = $AreaAdministratives->get($area_administrative_id)->code;

                $itemAreaAdministratives = $Areas
                    ->find()
                    ->where(['code'=>$codeAdministratives])
                    ->first();
                if(count($itemAreaAdministratives) > 0){
                    $area_administrative_id = $itemAreaAdministratives->id;
                    if($area_administrative_id != $parent_id){
                        $validationErrorMsg = $model->getMessage('Institution.Institutions.area_id.configuredAreaAdministrative');
                    }
                }
            }
        ...
        }else if ($globalData['field'] == 'area_aimak_id'){

            $validateAreaAimakLevel = $ConfigItems->value('institution_validate_area_aimak_level_id');
            $Areas = TableRegistry::get('Area.Areas');
            $AreaLevels = TableRegistry::get('Area.AreaLevels');

            $checkLevel = $AreaLevels->get($Areas->get($check)->area_level_id)->level;
            if ($checkLevel != $validateAreaAimakLevel){
                $configuredAreaLevel = $AreaLevels->find()
                    ->where([
                        $AreaLevels->aliasField('level') => $validateAreaLevel
                    ])
                    ->first();
                $validationErrorMsg = $model->getMessage('Institution.Institutions.area_aimak_id.configuredArea', ['sprintf' => [$configuredAreaLevel->name]]);
            }else{
                $Areas = TableRegistry::get('Area.Areas');
                $parent_id = $Areas->get($check)->parent_id;
                $area_id = $globalData['data']['area_id'];
                if($area_id != $parent_id){
                    $validationErrorMsg = $model->getMessage('Institution.Institutions.area_aimak_id.configuredAreaAimak');
                }
            }
        }
    ...
    public static function checkMaxStudentsPerClass($capacity, array $globalData)
    {
        ...
        if ($capacity < 0 || $capacity == 0) {
            $errorMsg = $model->getMessage('Institution.InstitutionClasses.capacity.ruleCheckMinStudentsPerClass');
            return $errorMsg;
        }
        elseif($capacity > $maxCapacity){
    ...
    public static function compareDate($field, $compareField, $equals, array $globalData)
    {
        ...
        if ($compareField) {
            ...
            if (!is_bool($result)) {
                ...
                return (!$result) ? $fieldName . __(' should be earlier than ') . $compareFieldName : true;
            }
        } else {
            return true;
        }
    ...
    public static function compareTime($field, $compareField, $equals, array $globalData)
    {
        ...
        if ($compareField) {
            ...
            if (!is_bool($result)) {
                ...
                return (!$result) ? __(Inflector::humanize($compareField). __(' should be on a later ') . $type) : true;
            }
        } else {
            return true;
        }
    ...
    public static function checkInputWithinRange($field, $field_name, $start_date, $end_date)
    {
        ...
        if ($givenDate > $startDate && $givenDate < $endDate) {
            ...
            return __(Inflector::humanize($field_name)).__(' is not within date range of ') .$start_date.__(' and ').$end_date;
        }
    }
    ...
    public static function checkInstitutionClassesTypes($field, array $globalData)
    {
        $model = $globalData['providers']['table'];
        $InstitutionClasses = TableRegistry::get('Institution.InstitutionClasses');
        $InstitutionTypes = TableRegistry::get('Institution.Types');

        $institutionTypes = $InstitutionClasses->find('list', [
            'keyField' => 'id',
            'valueField' => 'institution.institution_type_id'
        ])
            ->contain('Institutions')
            ->where([
                $InstitutionClasses->aliasField('institution_classes_view_id') => $globalData['data']['id']
            ])
            ->toArray();
        if(!empty($institutionTypes)) {  // not empty means position title in use & there's associated grade
            $postInstitutionTypes = $globalData['data']['institution_types']['_ids'];
            if (array_intersect($institutionTypes, $postInstitutionTypes) == $institutionTypes) {
                return true;
            } else {
                $arr = array_diff($institutionTypes, $postInstitutionTypes);
                $results = $InstitutionTypes->find()
                    ->where([$InstitutionTypes->aliasField('id IN ') => $arr])
                    ->extract('name')
                    ->toArray();
    ...
    public static function checkInstitutionsTypes($field, array $globalData)
    {
        $model = $globalData['providers']['table'];
        $Institutions = TableRegistry::get('Institution.Institutions');
        $InstitutionTypes = TableRegistry::get('Institution.Types');

        $institutionTypes = $Institutions->find()
            ->distinct('institution_type_id')
            ->where([
                $Institutions->aliasField('institution_view_id') => $globalData['data']['id']
            ])
            ->extract('institution_type_id')
            ->toArray();
        if(!empty($institutionTypes)) {  // not empty means position title in use & there's associated grade
            $postInstitutionTypes = $globalData['data']['institution_types']['_ids'];
            if (array_intersect($institutionTypes, $postInstitutionTypes) == $institutionTypes) {
    ...
    public static function checkInstitutionPeriodTypes($field, array $globalData)
    {
        $model = $globalData['providers']['table'];
        ...
        $institutionAttendanceTypes = $InstitutionAttendance->find('list', [
            'valueField' => 'institution.institution_type_id'
        ])
            ->contain('Institutions')
            ->where([
                $InstitutionAttendance->aliasField('institution_period_id') => $globalData['data']['id']
            ])
            ->toArray();

        $InstitutionAggregatedDataVpoTypes = $InstitutionAggregatedDataVpo->find('list', [
            'valueField' => 'institution.institution_type_id'
        ])
            ->contain('Institutions')
            ->where([
                $InstitutionAggregatedDataVpo->aliasField('institution_period_id') => $globalData['data']['id']
            ])
            ->toArray();

        $institutionTypes = array_merge($institutionAttendanceTypes,$InstitutionAggregatedDataVpoTypes);
