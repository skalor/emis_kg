...
class LocalesTable extends AppTable
{
    ...
    public function getAllExceptMain(): array
    {
        static $locales;
    
        $locales = $locales ?? $this->getList()
                ->where([$this->aliasField('name') . ' <> ' => 'English'])
                ->toArray();
    
        return $locales;
    }
    
    public function getLocaleTitle(int $id): ?string
    {
        static $locales;
        
        $locales = $locales ?? $this->getList()->where(['name <>' => 'English'])->toArray();
