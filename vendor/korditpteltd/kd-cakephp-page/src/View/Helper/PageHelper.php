...
class PageHelper extends Helper
{
    ...
    public $includes = [
        'datepicker' => [
            ...
            'js' => [
                'Page.../plugins/datepicker/js/bootstrap-datepicker.min',
                'Page.../plugins/datepicker/locales/bootstrap-datepicker.ru.min',
                'Page.../plugins/datepicker/locales/bootstrap-datepicker.kg.min'
            ],
        ...
        foreach ($fields as $field => $attr) {
            ...
            switch ($controlType) {
                ...
                case 'textarea':
                    ...
                    // fall through
                default:
                    $html .= sprintf($row, $label, __($value));
                    break;
            }
        }
        return $html;
...
EOT;
    ...
    private function select(array $field, $data)
    {
        ...
        $options['options'] = $this->translateOptions($options['options']);
        $cakephpVersion = Configure::version();
        if (version_compare($cakephpVersion, '3.4.0', '>=')) {
            return $this->Form->control($field['attributes']['name'], $options);
        } else {
    ...
    private function date(array $field, $data)
    {
        ...
        $dateFormat = isset($field['attributes']['format']) ? $field['attributes']['format'] : 'dd-MM-yyyy';
        $minViewMode = isset($field['attributes']['minViewMode']) ? $field['attributes']['minViewMode'] : false;
        $maxViewMode = isset($field['attributes']['maxViewMode']) ? $field['attributes']['maxViewMode'] : false;
        ...
        if (!empty($value)) {
            if ($value instanceof Date) {
                $options['value'] = $value->i18nFormat($dateFormat);
            } else {
                $options['value'] = (new Date($value))->i18nFormat($dateFormat);
            }
        } else {
            if ($required) {
                $options['value'] = (new Date())->i18nFormat($dateFormat);
            }
        }
